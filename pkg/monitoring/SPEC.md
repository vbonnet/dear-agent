...
**NFR-1-1** The File watcher shall process events with < 100ms latency
**NFR-1-2** The Validation shall complete in < 5 seconds for typical projects
**NFR-1-3** The Event log writes shall be direct file appends (no buffering)
- **NFR-1-4**: Fallback verification (file walking, test execution) may take longer

### NFR-2: Reliability

**NFR-2-1** The Monitoring failures shall NOT block sub-agent execution
**NFR-2-2** The Missing event log shall fall back to direct verification (git log, file count, test execution)
**NFR-2-3** The Sparse event log (only git commits) shall NOT prevent validation
**NFR-2-4** The Git hook installation shall preserve existing hooks via backup
**NFR-2-5** The File watcher shall handle inotify limit errors gracefully
**NFR-2-6** The Validation shall work even without EventBus events (fallback-first design)
...
**NFR-4-1** The Event logs shall be written to secure temporary directories
**NFR-4-2** The Git hooks shall be executable only by owner (0755)
**NFR-4-3** The No sensitive data shall be logged in events

### NFR-5: Maintainability

**NFR-5-1** The Validation thresholds shall be externally configurable
**NFR-5-2** The Output patterns shall be extensible via AddPattern()
**NFR-5-3** The File filters shall be extensible via AddFilter()
**NFR-5-4** The All components shall be testable in isolation
