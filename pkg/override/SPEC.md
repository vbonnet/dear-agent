...
**OVR-15** When the privileged ledger helper is installed, the system shall require the operator to confirm the complete artifact and fixed-installer SHA-256 values, reject a passwordless probe of that exact installer, copy the helper and both caller launchers into unique root-owned staging files, verify their digest or code identities against the confirmed values, activate only the root-owned helper and policy set inside one rollback-protected non-caching privileged command, and activate the verified caller launchers afterward only from the unprivileged operator shell so a replaceable user directory is never resolved by root.
