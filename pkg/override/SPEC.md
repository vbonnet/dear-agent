...
**OVR-15** When the privileged ledger helper is installed, the system shall hold one host-global exclusive operator-owned lock across the complete user/root transaction, require the operator to confirm the complete artifact and fixed-installer SHA-256 values, reject a passwordless probe of that exact installer, copy the helper and both caller launchers into unique staging files, verify their digest or code identities against the confirmed values, back up and activate the verified caller launchers only from the unprivileged operator shell so a replaceable user directory is never resolved by root, and then activate the root-owned helper and policy set inside one rollback-protected non-caching privileged command; authentication, activation, or signal failure shall restore the prior launcher set while the privileged transaction restores the prior root-owned set, and only a transaction-specific root-owned commit receipt may suppress compensating launcher rollback after an interrupted wait.

**OVR-16** When the macOS system audit is installed, the system shall require the operator to confirm the executable, rendered LaunchDaemon, transaction-helper, and fixed bootstrap SHA-256 values, reject a passwordless probe of that exact bootstrap command, stage the helper only beneath verified root-owned non-writable non-symlink ancestry, and run the verified helper's complete staging, digest and plist validation, backup, and activation inside one fresh non-caching privileged command; any HUP, INT, or TERM received by the recipe, bootstrap, or helper shall propagate to the transaction, restore every prior artifact during partial activation, remove newly introduced artifacts, and exit nonzero without resuming the transaction.

**OVR-17** When the Linux system audit is installed, the system shall require the operator to confirm the executable, service, timer, privileged transaction-helper, and fixed bootstrap SHA-256 values, reject a passwordless probe of that exact bootstrap command, stage the helper only beneath verified root-owned non-writable non-symlink ancestry, and run the verified helper's complete staging, digest verification, backup, activation, and system-manager reload inside one fresh non-caching privileged command; any HUP, INT, or TERM received by the recipe, bootstrap, or helper shall propagate to the transaction, restore every prior artifact during partial activation, remove newly introduced artifacts, and exit nonzero without resuming the transaction.
