...
// is signalled by closing its input channel; the canceled status is
...
func (e *sessionExecutor) Cancel(ctx context.Context, reqCtx *a2asrv.RequestContext, queue eventqueue.Queue) error {
	if v, ok := e.sessions.LoadAndDelete(reqCtx.TaskID); ok {
		run := v.(*runtime)
		// Closing nextInput unblocks any AskInput call that was waiting.
		// A handler that respects ctx will then return; the goroutine
		// writes a Failed event in that path, but we override here with
		// Canceled to honour the client's intent.
		safeClose(run.nextInput)
	}
