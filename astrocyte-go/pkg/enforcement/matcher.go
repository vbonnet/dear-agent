...
type MatchResult struct {
	...
	Groups     []string // Captured groups from the regex
	StartIndex int      // Start position of match
	EndIndex   int      // End position of match
}
...
// QuickMatch is a convenience function to check if a pattern matches without creating a Matcher.
func QuickMatch(pattern, input string) (bool, error) {
	return regexp.MatchString(pattern, input)
}

// ExtractGroups extracts named or numbered groups from a match.
