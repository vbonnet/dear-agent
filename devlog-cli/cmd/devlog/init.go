...
import (
	...
	"github.com/vbonnet/ai-tools/devlog/internal/config"
...
func runInit(cmd *cobra.Command, args []string) error {
	...
	out.Progress(fmt.Sprintf("Initializing devlog workspace: %s", workspaceName))
	...
	if _, err := os.Stat(configPath); err == nil && !initForce {
		return fmt.Errorf("workspace already initialized (config file exists at %s)\n"+
			"Use --force to overwrite existing configuration", configPath)
	}

	// Create .devlog directory
	if err := os.MkdirAll(devlogDir, 0755); err != nil {
		return fmt.Errorf("failed to create .devlog directory: %w", err)
	}
	out.Success("Created .devlog directory")
...
` + string(data)
	...
	out.Success("Created .devlog/config.yaml")
...
`
	...
	if err := os.WriteFile(localExamplePath, []byte(localExampleContent), 0644); err != nil {
		out.Error(fmt.Sprintf("Warning: failed to write config.local.yaml.example: %v", err))
	} else {
		out.Success("Created .devlog/config.local.yaml.example")
	}
...
`, workspaceName)
	...
	if err := os.WriteFile(readmePath, []byte(readmeContent), 0644); err != nil {
		out.Error(fmt.Sprintf("Warning: failed to write README.md: %v", err))
	} else {
		out.Success("Created .devlog/README.md")
	}
...
`
	...
	if err := os.WriteFile(gitignorePath, []byte(gitignoreContent), 0644); err != nil {
		out.Error(fmt.Sprintf("Warning: failed to write .gitignore: %v", err))
	} else {
		out.Success("Created .devlog/.gitignore")
	}

	// Success summary
	out.Info("")
	out.Success(fmt.Sprintf("Initialized devlog workspace: %s", workspaceName))
	out.Info("")
	out.Info("Next steps:")
	out.Info("  1. Edit .devlog/config.yaml to configure your repositories")
	out.Info("  2. Run 'devlog sync' to clone repos and create worktrees")
	out.Info("  3. (Optional) Create .devlog/config.local.yaml for personal overrides")
