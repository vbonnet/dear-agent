...
func TestParseWorktreeList_Single(t *testing.T) {
	output := `worktree /home/user/repo/main
HEAD abc123def456
...
`
	...
	wt := worktrees[0]
	if wt.Path != "/home/user/repo/main" {
		t.Errorf("worktree.Path = %s, want /home/user/repo/main", wt.Path)
	}
	if wt.Name != "main" {
...
func TestParseWorktreeList_DetachedHead(t *testing.T) {
	output := `worktree /home/user/repo/detached
