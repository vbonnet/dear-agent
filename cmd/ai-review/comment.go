...
func buildComment(outcome Outcome, synthesis string, reports []dimensionReport, override bool, triggers []string) string {
	...
	fmt.Fprintf(&b, "> Automated 5-dimension review per [REVIEW.md](REVIEW.md). This check is **fail-closed**: any non-approved outcome fails it. It is currently **not** a required status check (paused 2026-07-27), so it does not by itself block merge.\n\n")
	if len(triggers) > 0 {
	...
	fmt.Fprintf(&b, "<sub>Review gate per REVIEW.md §2/§5 (paused 2026-07-27 — not a required check; see §4).</sub>\n")
	return b.String()
}
...
func forkComment() string {
	return commentMarker + "\n## 🔴 AI Code Review — needs-human-review\n\n" +
		...
		"<sub>Review gate per REVIEW.md §2/§5 (paused 2026-07-27 — not a required check; see §4).</sub>\n"
}
