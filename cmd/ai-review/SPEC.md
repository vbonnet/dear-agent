...
**AIREV-26** When the review cannot run for the sole reason that no reviewer credential is configured on a same-repository, non-override run, the command shall post its needs-human-review evidence and exit with the distinct blocking cannot-run status 78, and only the trusted workflow shall translate exactly that status into a neutral-with-warning published verdict; every other failure, including fork pull requests, plan construction errors, expired deadlines, and override audit-comment failures, shall keep an undifferentiated non-zero status and remain fail closed.
