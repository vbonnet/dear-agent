...
**RESOLVE-REVIEW-THREADS-80** When a reply could create an outstanding continuation obligation, the system shall preflight a persistent 32-byte signing key before posting; use the selected XDG state root or the documented home-state fallback; require every command-managed directory to remain private and non-symlink; require the key leaf to be a non-symlink regular file of exact length and mode `0600`; atomically create it without replacing an existing leaf; synchronize every managed directory entry with its parent on every issuance preflight, including retry after an earlier synchronization failure; synchronize the key and containing directory; and fail before provider mutation when any invariant or durability operation fails.

**RESOLVE-REVIEW-THREADS-81** When `continue-resolve` receives a receipt, the system shall load the existing issuer key, authenticate and canonically decode the receipt, and compare its provider host with the normalized effective ambient GitHub host before reading the body source or contacting the provider; the system shall treat the same-user key as a local issuance boundary rather than protection from an operating-system user who can read or replace that state or invoke `gh` directly.

**RESOLVE-REVIEW-THREADS-82** When a structurally valid continuation cannot be authenticated because the original key or selected state root is missing, unreadable, replaced, or changed, or when its bound provider host differs from the ambient host, the system shall retain both the exact receipt and actual source, issue no provider request or mutation, instruct restoration of the exact issuing host, state root, and key, and forbid regenerating or overwriting the key; malformed, non-canonical, or bad-authenticator input may instead be described as an invalid receipt.
...
**RESOLVE-REVIEW-THREADS-87** While owner-private continuation key state and durable directory entries cannot be established on a non-Unix platform, the system shall reject `reply-resolve` continuation preparation before issuing a reply mutation and shall reject `continue-resolve` receipt loading before reading the body source or contacting the provider; support for that platform shall require native privacy and runtime durability proof rather than POSIX mode bits or cross-compilation alone.
