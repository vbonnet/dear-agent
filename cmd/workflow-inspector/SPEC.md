...
**WORKFLOW-INSPECTOR-05** When a `state` query parameter is provided on the list route, the system shall filter listed runs by that workflow state, and when WFLOW-13 rejects that filter the system shall return HTTP 400 rather than an internal error.
