...
merged PR's head, and that pull request merged into a protected branch. A
committer date cannot carry that proof, because a force-push after the merge
can put an older commit — carrying unmerged work — on the branch tip. Nor can
a merge into an ephemeral parent: that parent can itself be closed unmerged
...
**BRR-03** When a branch's most-recently-merged pull request merged into a protected branch and records a head commit equal (case-insensitively) to the branch's tip commit SHA, the command shall classify the branch as `safe_delete`.

**BRR-04** When a branch's most-recently-merged pull request records a head commit that differs from the branch's tip commit SHA, records no head commit at all, or merged into a base that is not protected, the command shall classify the branch as `review_new_commits_after_merge`, because a merge into an ephemeral branch does not prove the content survives that branch's own deletion.
...
**BRR-16** When a protected-branch entry is a GitHub Actions branch filter containing pattern syntax, the command shall protect every branch the filter matches under GitHub's filter grammar (`*` within one path segment, `**` across segments, `?` for zero or one of the preceding character, `+` for one or more of the preceding character, `[...]` character classes, `\` escapes), rather than only a branch whose literal name equals the filter; shall treat a negated (`!`) filter as protecting nothing; and shall treat any filter it cannot translate as protecting the branch rather than leaving it deletable.
