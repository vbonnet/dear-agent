...
func run(args []string, stdout, stderr io.Writer, now time.Time) int {
	...
	switch args[0] {
	case "authorize":
		err = runAuthorize(args[1:], stdout, now)
	case "verify-authorization":
		err = runVerifyAuthorization(args[1:], stdout, now)
	case "receipt":
		err = runReceipt(args[1:], stdout, now)
	case "verify-receipt":
		err = runVerifyReceipt(args[1:], stdout)
	case "help", "-h", "--help":
		usage(stdout)
		return 0
	default:
...
func runAuthorize(args []string, stdout io.Writer, now time.Time) error {
...
func runVerifyAuthorization(args []string, stdout io.Writer, now time.Time) error {
	...
	if err := fs.Parse(args); err != nil || fs.NArg() != 0 || anyEmpty(*claimsPath, *expectedPath) {
		return errors.New("invalid input")
	}
...
func runReceipt(args []string, stdout io.Writer, now time.Time) error {
	...
	keyPath := fs.String("hmac-key", "", "base64url HMAC key file")
	if err := fs.Parse(args); err != nil || fs.NArg() != 0 || anyEmpty(*contextPath, *authorizationPath, *keyPath) {
		return errors.New("invalid input")
	}
...
func runVerifyReceipt(args []string, stdout io.Writer) error {
	fs := flag.NewFlagSet("verify-receipt", flag.ContinueOnError)
	fs.SetOutput(io.Discard)
	receiptPath := fs.String("receipt", "", "canonical receipt claims")
	expectedPath := fs.String("expected", "", "current expected public bindings")
	if err := fs.Parse(args); err != nil || fs.NArg() != 0 || anyEmpty(*receiptPath, *expectedPath) {
		return errors.New("invalid input")
	}
