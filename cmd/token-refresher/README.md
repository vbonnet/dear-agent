...
- **launchd (idle backstop):** schedule `token-refresher -cadence` so the
  ...
  `~/.local/state/dear-agent/token-refresher-cadence-audit.jsonl`. Manual
  `-check`, `-force`, and ordinary invocations with default audit wiring keep
  using `token-refresher-audit.jsonl`, so unrelated operator commands do not
  make a stalled scheduler look alive. `-audit-log` remains an explicit
  operator override; the cadence file is schedule-specific wiring, not
  tamper-proof proof that launchd fired autonomously.
