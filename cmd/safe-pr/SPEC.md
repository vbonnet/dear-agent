...
**SAFE-PR-16** When pull request creation launches preflight, GitHub mutation, or CI-discovery commands, the system shall attach the active worktree transaction guard so child process lifetime remains part of the protected transaction after abrupt parent termination. For preflight, the guard runner shall require its argument to match its current working directory and shall close the inherited descriptor before it launches nested build or test processes, so detached test helpers cannot retain the transaction after the guard exits.
