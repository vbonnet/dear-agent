...
func TestDispatchCandidates_SkipsDeterministicSpawnFailure(t *testing.T) {
	...
	got := dispatchCandidates(context.Background(), candidates, "opus-200k", 0, false, &out, &errOut)
	if got != 2 {
...
func TestDispatchCandidates_CapsSuccessfulDispatches(t *testing.T) {
	...
	var out, errOut bytes.Buffer
	got := dispatchCandidates(context.Background(), candidates, "opus-200k", 2, false, &out, &errOut)
	if got != 2 {
...
func TestDispatchCandidates_CapCountsOnlySuccesses(t *testing.T) {
	...
	var out, errOut bytes.Buffer
	got := dispatchCandidates(context.Background(), candidates, "opus-200k", 1, false, &out, &errOut)
	if got != 1 {
...
func TestDispatchCandidates_UncappedByDefault(t *testing.T) {
	...
	spawnSession = func(ctx context.Context, name, model string) error { return nil }
	sendPrompt = func(ctx context.Context, name, prompt string) error { return nil }
	...
	for _, maxDispatch := range []int{0, 5} {
		var out, errOut bytes.Buffer
		if got := dispatchCandidates(context.Background(), candidates, "opus-200k", maxDispatch, false, &out, &errOut); got != 3 {
...
func TestDispatchCandidates_DryRunHasNoWorkerCap(t *testing.T) {
	...
	var out, errOut bytes.Buffer
	got := dispatchCandidates(context.Background(), candidates, "opus-200k", 0, true, &out, &errOut)
...
func TestDispatchCandidates_StopsOnCanceledContext(t *testing.T) {
	...
	var out, errOut bytes.Buffer
	got := dispatchCandidates(ctx, candidates, "opus-200k", 0, true, &out, &errOut)
	if got != 0 {
