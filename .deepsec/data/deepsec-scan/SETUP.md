# Agent setup for `deepsec-scan`

This is a deepsec scanning workspace. Project `deepsec-scan` was just registered
(target: `..`). Setup is incomplete — `data/deepsec-scan/INFO.md`
...
2. **Fill in `data/deepsec-scan/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id deepsec-scan
pnpm deepsec process --project-id deepsec-scan
```
