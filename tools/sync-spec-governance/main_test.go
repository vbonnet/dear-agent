...
var testSkillNames = specgovernance.Names()
...
func TestSyncWritesAndChecksProjections(t *testing.T) {
	...
	for _, name := range canonicalEARSSources {
		path := filepath.Join(root, "internal", "earslint", name)
		data, err := os.ReadFile(path)
		if err != nil {
			t.Fatalf("read generated EARS projection %s: %v", name, err)
		}
		if !bytes.HasPrefix(data, []byte(generatedGoMarker+"\n\npackage earslint")) {
			t.Fatalf("generated EARS projection %s does not retain the canonical package: %q", name, data)
		}
	}
	data, err := os.ReadFile(filepath.Join(root, "internal", "earslint", "SPEC.md"))
	if err != nil || !bytes.Equal(data, []byte(generatedEARSProjectionSPEC)) {
		t.Fatalf("generated EARS projection SPEC differs: data=%q err=%v", data, err)
	}
	for _, want := range []string{
		"non-normative for grammar behavior",
		"No portable behavior is owned here.",
	} {
		if !bytes.Contains(data, []byte(want)) {
			t.Errorf("generated EARS projection SPEC does not contain %q:\n%s", want, data)
		}
	}
}

func TestSyncCheckReportsStaleEARSProjection(t *testing.T) {
	root := t.TempDir()
	writeCanonicalSkills(t, root)
	if err := sync(root, false); err != nil {
		t.Fatalf("sync() error = %v", err)
	}
	writeFile(t, root, "spec-governance/earslint/config.go", "package earslint\n\ntype Config struct{ Updated bool }\n")
	if err := sync(root, true); err == nil || !strings.Contains(err.Error(), "internal/earslint/config.go") || !strings.Contains(err.Error(), "stale") {
		t.Fatalf("sync(check) error = %v, want stale EARS projection error", err)
	}
}

func TestSyncRequiresExplicitRemovalForStaleEARSProjection(t *testing.T) {
	root := t.TempDir()
	writeCanonicalSkills(t, root)
	if err := sync(root, false); err != nil {
		t.Fatalf("sync() error = %v", err)
...
func TestSyncRejectsUnprojectedCanonicalEARSFile(t *testing.T) {
	root := t.TempDir()
	writeCanonicalSkills(t, root)
	writeFile(t, root, "spec-governance/earslint/extra.go", "package earslint\n")
	if err := sync(root, true); err == nil || !strings.Contains(err.Error(), "want fixed projection set") {
		t.Fatalf("sync(check) error = %v, want fixed EARS source-set rejection", err)
	}
}
...
func TestSyncRejectsFullyDeclaredAdditionalCanonicalSkill(t *testing.T) {
	...
	fullyDeclared := make([]string, len(testSkillNames)+1)
	copy(fullyDeclared, testSkillNames)
	fullyDeclared[len(testSkillNames)] = "review-spec"
...
func writeCanonicalSkills(t *testing.T, root string) {
	...
	writeCanonicalEARS(t, root)
}

func writeCanonicalEARS(t *testing.T, root string) {
	t.Helper()
	writeFile(t, root, "spec-governance/earslint/config.go", "package earslint\n\ntype Config struct{}\n\nfunc DefaultConfig() Config { return Config{} }\n")
	writeFile(t, root, "spec-governance/earslint/config_test.go", "package earslint\n")
	writeFile(t, root, "spec-governance/earslint/earslint.go", "package earslint\n")
	writeFile(t, root, "spec-governance/earslint/earslint_test.go", "package earslint\n")
}
