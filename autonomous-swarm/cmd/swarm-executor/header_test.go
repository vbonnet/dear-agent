...
func TestHeaderFormat(t *testing.T) {
	...
	// Verify format: "swarm-executor <version> (<path>)"
	if !strings.HasPrefix(header, "swarm-executor ") {
		t.Errorf("Header should start with 'swarm-executor ', got: %s", header)
	}
...
func formatHeader(version, executable string) string {
	return "swarm-executor " + version + " (" + executable + ")"
}
