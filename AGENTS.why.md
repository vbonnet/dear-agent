...
| 2026-05-15 | Corrected the dogfooding mandate's stale acceptance-criteria startup guidance | Dogfooding the handoff-confidence work surfaced that acceptance criteria needed to be explicit at task start instead of living only in ad hoc prompts. `agm acceptance show` now exists and reads `.dear-agent.yml` via `pkg/acceptance`; `agm session new` also announces configured criteria when a session starts. `agm session new` and `agm admin doctor` were verified to exist and kept. See `vbonnet/engram-research` `retrospectives/2026-05-15-handoff-confidence-and-agm-cli-drift.md`. |
...
| 2026-06-30 | Added Codex, AGY, and OpenCode root instruction shims plus an import-contract test | The parity work made the 2026-06-24 intent executable. `CLAUDE.md`, `GEMINI.md`, `CODEX.md`, `AGY.md`, and `OPENCODE.md` now all import `AGENTS.md`, and `internal/instructions` tests guard both root and scoped entrypoints so shared policy does not drift back into harness-specific files. |
