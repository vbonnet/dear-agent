...
| 2026-06-09 | Followed up the merged anti-stall spec (PR #278, `docs/design/anti-stall.md` + CLAUDE.md §Anti-Stall, Beads `ce-mzm`) with the two completeness items #278 omitted: this changelog row, and a reference-integrity test (`pkg/gracefulexit/antistall_reference_test.go`) | #278 added the instruction-tier rule but skipped the tier convention's two trailing obligations. **(a) This row.** Every instruction-tier CLAUDE.md change is logged here so the rationale survives — #278 broke that. The spec consolidates guidance repeatedly re-issued per-prompt ("never ask 'should I keep going?'", "execute the backlog continuously", "'nothing found' is valid"), the same per-prompt-rot ADR-018 and the output-routing rows record; publishing once and referencing from CLAUDE.md is the structural fix. **(b) The test.** A spec only works while CLAUDE.md keeps pointing at it and the spec keeps stating its directives; a dangling reference or gutted spec silently retires the rule. The repo's recurring failure is exactly that — a doc lands, the wiring that makes agents see it rots untested. The test asserts the spec exists with all five directives + the boundary section, and that CLAUDE.md still references it. Homed in `pkg/gracefulexit` (not a new package — avoids a `cmd/n` dead-package regression) because the spec's directive 2 *is* this package's no-overfit guardrail; the two are already coupled. Instruction tier only, consistent with the deferred-enforcement stance — the test is a link-rot guard, not a behavioural gate. |
| 2026-06-12 | Added the **enforcement tier** for the wayfinder-only PR lifecycle: `cmd/safe-pr` + `internal/safepr` (the sanctioned wrapper), a repo-scoped exit-2 `PreToolUse` hook `.claude/hooks/pretool-pr-guard`, `Bash(safe-pr:*)` allow-listed in `.claude/settings.json`, and CLAUDE.md §PR Lifecycle (Beads `ce-p17s`; wayfinder session `87a5378c`) | The 2026-05-29 principle-9 row deferred a PR wrapper; the leak then materialized exactly as the output-routing rows predicted: burndown workers sprayed raw `gh pr create` PRs ~10× faster than the serial merge pipeline (required checks + linear history) could land them, with zero wayfinder/bead attribution and zero telemetry (2026-06-11/12 logjams, duplicate PRs). Instruction tier (CLAUDE.md DoD) demonstrably failed, so this escalates to enforcement. **Design choices and their why:** (a) *hard block, not a nudge* — unlike pretool-spawn-routing, the failure is proven and costly, so the hook exits 2 with principle-2 positive guidance (the safe-pr command line, the why, the escalation footer); (b) *wrapper carries the trace by construction* — safe-pr refuses to run without an in_progress `WAYFINDER-STATUS.md` (or an audited `--emergency --reason`), stamps the session into the PR body/close comment, audit-logs JSONL to `~/.local/state/dear-agent/safe-pr.log`, and emits an OTel span — attribution is unforgeable-by-omission rather than remembered; (c) *interactive/unstampable gh forms refused* (`--web`, `--fill*`, `--body-file`, `--editor`, missing `--title`) so the run is deterministic and headless-safe; (d) *scope* (principle 1): only `create|close|reopen` are denied — read-only pr verbs and the already-governed `gh pr merge` pass; in-code creators (`pkg/selfimprove`, agm evaluation) and the burndown prompt are scoped follow-ups (ce-ijsq, ce-jzqa, ce-gzmr), global rollout is ce-20en. **Honest limitations** (per the 2026-06-03 precedent): the hook matches literal command forms incl. launcher prefixes (`gtimeout`/`env`/`sudo`) and line continuations, but `bash -c`, command substitution, on-disk scripts, gh aliases, and `gh api -X POST /repos/…/pulls` bypass it — a best-effort net for the cooperative-but-forgetful agent, not a security boundary; the permission classifier stays the backstop for unrecognized gh writes. Refused requests are audit-logged too (not just executions). `reopen` is denied with a points-at-a-human message, not routed to safe-pr (reopening is a human decision). Allow-listing `Bash(safe-pr:*)` deliberately grants a *session-traced* close capability where raw `gh pr close` had none; safe-pr refuses `--repo`/`-R` so the wrapper stays pinned to the cwd repo. Size deviation acknowledged: hook ~100 lines (guideline <50) for the launcher/continuation matcher + verb-aware guidance, `cmd/safe-pr` main 217 lines (guideline <200) with policy split into `internal/safepr` per the safe-push precedent. |
