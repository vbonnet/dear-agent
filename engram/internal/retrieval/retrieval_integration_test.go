...
func TestRetrieval_WithConfig_Integration(t *testing.T) {
	...
	// Create test engrams using helper
	createTestEngrams(t, tmpDir)
	...
	t.Run("custom_search_paths", func(t *testing.T) {
		...
		// Verify results are from our temp directory using helper
		assertResultsInDirectory(t, results, tmpDir)
	})
	...
	t.Run("config_overrides", func(t *testing.T) {
		...
		// Verify all results have go tag using helper
		assertResultsHaveTag(t, resultsGo, "go")
	})
}

// Test helper functions for TestRetrieval_WithConfig_Integration

// createTestEngrams creates test engram files in the specified directory
func createTestEngrams(t *testing.T, tmpDir string) {
	t.Helper()
	testEngrams := []struct {
		filename string
		content  string
...
// assertResultsInDirectory verifies all result paths are in the expected directory
func assertResultsInDirectory(t *testing.T, results []*SearchResult, expectedDir string) {
	t.Helper()
...
// assertResultsHaveTag verifies all results have the specified tag
func assertResultsHaveTag(t *testing.T, results []*SearchResult, tag string) {
	t.Helper()
	for _, r := range results {
		hasTag := false
		for _, resultTag := range r.Engram.Frontmatter.Tags {
			if resultTag == tag {
				hasTag = true
				break
			}
		...
		if !hasTag {
			t.Errorf("Result %q does not have '%s' tag", r.Engram.Frontmatter.Title, tag)
		}
	}
}
