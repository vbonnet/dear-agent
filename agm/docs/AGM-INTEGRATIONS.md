...
pipeline {
    ...
    stages {
        stage('Setup AGM') {
            steps {
                sh 'go install github.com/vbonnet/dear-agent/agm/cmd/agm@latest'
            }
        }
        ...
        stage('Run Tests') {
            steps {
                sh 'agm send msg ci-build-${BUILD_NUMBER} --prompt "Run test suite"'
            }
        }
...
jobs:
  agm-test:
    ...
    steps:
      ...
      - run:
          name: Install AGM
          command: go install github.com/vbonnet/dear-agent/agm/cmd/agm@latest
      - run:
          name: Install tmux
...
# Send test prompt
agm send msg "$SESSION" --prompt "Run full test suite and report results"
...
# Send research query
agm send msg "$SESSION" --prompt-file ~/research/daily-topics.txt
