...
| Unarchive overlaps archive cleanup | Serialize both operations with the stable session-ID lifecycle lock and reload lifecycle under that lock before reactivation, so cleanup cannot remove a sandbox after the session becomes active |
...
10. **Archive cleanup retries only proven transient holders** — a refusal naming a specific live process is retried only before one absolute bounded process-grace deadline. The remaining budget is propagated into each in-flight process and mount scan and re-checked before removal, the upper-layer settings snapshot is refreshed before every attempt, and every path, process, unmount, and mount gate is re-run in that order without a caller-side pre-unmount. Persistent holders, expired or unreadable process state, invalid paths, or surviving mounts preserve the sandbox.
11. **Archive and unarchive share one lifecycle transaction** — both operations serialize on the stable session ID and reload lifecycle under that lock, so unarchive cannot reactivate a session while archive still owns destructive cleanup.
