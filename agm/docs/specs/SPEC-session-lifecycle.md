...
| Archive sandbox is held by an exiting child process | Retry before one absolute bounded process-grace deadline, re-running all cleanup safety gates, canceling in-flight process and mount scans at that deadline, and re-checking the budget before removal; preserve it if the holder persists |
