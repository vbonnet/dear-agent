...
11. **Archive and every reactivation path share one lifecycle transaction** — archive, unarchive, and administrative reconciliation serialize on the stable session ID and reload lifecycle under that lock. Reconciliation also revalidates its tmux mismatch while holding the lock, so no stale repair can reactivate a session while archive owns destructive cleanup or overwrite lifecycle work completed while it waited.
