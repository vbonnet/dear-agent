...
## Regression #2: Redundant WaitForClaudePrompt Timeout (2026-03-25)

**Symptom**: `/rename` command not sent automatically in new sessions. Manual invocation required.

**Root Cause**: `InitSequence.sendRename()` called `WaitForClaudePrompt()` redundantly after `new.go:834` already verified the prompt. By the time the second check ran (after manifest creation, Dolt, git commit), the prompt had scrolled off the 50-line capture buffer, causing a 30s timeout.

**Exacerbating Factor**: Sandbox-by-default feature increased time between prompt checks, making scrolloff more likely.

**Fix**: Added `PromptVerified` flag to `InitSequence` struct. When set by caller, skip redundant prompt waits.

**Files Modified**:
- `internal/tmux/init_sequence.go` - Added flag, skip logic
...
## Regression #3: Trust Dialog Consuming /rename Keystrokes (2026-03-26)

**Symptom**: /rename command sent via tmux send-keys but not processed by Claude Code. Only /agm:agm-assoc visible in pane.

**Root Cause**: Each sandbox session gets a new UUID directory. Claude Code shows a trust dialog ("Do you trust the files in this folder?") for unknown directories. WaitForClaudePrompt false-positives on the prompt character in the dialog's option list, so /rename keystrokes were typed into the trust dialog and consumed.

**Fix**: Before sending /rename, init_sequence.go checks the last 10 lines of capture-pane output for trust dialog signatures ("Enter to confirm", "I trust this folder"). If detected, sends Enter to confirm trust, waits 3s for Claude to load, then re-waits for the real Claude prompt before sending /rename.

**Files Modified**:
- `internal/tmux/init_sequence.go` - Trust dialog detection and auto-confirmation
