...
| AGM-017 | 503 | `session/output_unavailable` | Session output unavailable | A live pane capture failed while the session is still running, so no durable capture can stand in for the current task | Transient — retry; if it persists, check the tmux socket and permissions |
