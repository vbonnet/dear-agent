...
**OPS-108** When post-archive sandbox removal is attempted and a sandbox directory was confirmed to exist, the system shall report `SandboxRemovalFailed` on `CleanupResult` if removal did not succeed, distinct from the case where no sandbox directory existed at all (never a failure); a CLI archive command shall surface `SandboxRemovalFailed` as a visible, impossible-to-miss error (not only a silently-omitted count) so a failed reap is never mistaken for a completed one. The underlying refusal shall be carried on `SandboxRemovalReason` and recorded in the cleanup audit log. Any retry guidance shall state that destructive sandbox GC is unavailable until authenticated session-store endpoint transport exists and, once that authority exists, that scheduled retry additionally requires the opt-in periodic launch agent.
...
**OPS-59** When `CreateSessionWithContext` advances a named session, the system shall require reservation-capable storage and atomically reserve its non-archived name before surface-specific workspace or permission preparation, bounded provider setup, tmux creation, or runtime launch, without a separate list-based availability preflight; an explicitly deferred current-pane launch shall consume that reservation in durable registration before its irreversible queue submission, while every other persistent launch shall prove readiness before consuming the reservation, and every persistent route shall run runtime completion only after registration. The deprecated Gemini wrapper is an explicit non-persistent exception: it synchronously owns the complete terminal lifecycle until the wrapped session exits, runs no shared runtime completion, creates no durable registration or provenance record, and holds then releases the name reservation only for that bounded wrapper invocation.
...
**OPS-80** When any shared creation surface launches AGY, the system shall use `CreateSessionWithContext` to resolve the existing workspace to one canonical physical path for locking, name reservation, tmux creation, launch, identity correlation, registration, and persisted metadata while holding the workspace-create lock across the fail-closed pre-launch snapshot through provider-identity registration and releasing it before surface-specific completion, including any blocking interactive attach, without surrendering normal rollback ownership.
...
**OPS-92** When a shared operation wraps a typed backend failure in its stable RFC 7807 error envelope, the in-process error chain shall preserve the original cause for `errors.Is` and `errors.As` while JSON and human-readable output retain the stable operation code and actionable detail.
...
**OPS-91** If `Sweep` is asked to execute without a caller-confirmed active-session set, then the system shall return `ErrActiveSessionsUnknown` before discovering or removing any worktree.
