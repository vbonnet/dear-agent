...
**OPS-59** When `CreateSessionWithContext` advances a named session, the system shall require reservation-capable storage and atomically reserve its non-archived name before surface-specific workspace or permission preparation, bounded provider setup, tmux creation, or runtime launch, without a separate list-based availability preflight; an explicitly deferred current-pane launch shall consume that reservation in durable registration before its irreversible queue submission, while every other launch shall prove readiness before consuming the reservation, and every route shall run runtime completion only after registration.
