...
**OPS-59** When `CreateSessionWithContext` advances a new session, the system shall atomically reserve its non-archived name before bounded Codex setup, tmux creation, or runtime launch; it shall then prove readiness, consume that reservation in the manifest-registration transaction, and run runtime completion.
...
**OPS-61** If any creation step fails after name reservation, manifest-directory creation, new tmux creation, or registration, the system shall release the reservation or delete the completed session registration, remove the newly-created tmux session, and remove only the manifest directory created by that operation.
