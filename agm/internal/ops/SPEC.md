...
**OPS-107** When archive sandbox cleanup receives a live-process refusal for a specific process immediately after owned session shutdown, the system shall retry only before an absolute bounded process-grace deadline while re-running every path, process, unmount, and mount safety gate; the system shall propagate the remaining shared budget into each in-flight process and mount scan and re-check it before removal, and persistent holders, expired or unreadable process state, bad paths, or surviving mounts shall preserve the sandbox.
