...
const (
	queueBusyTimeout           = 5 * time.Second
	queueBusyRetryInterval     = 10 * time.Millisecond
	queueSQLiteBusyPrimaryCode = 5
)

type queueSQLiteCodeError interface {
	error
	Code() int
}
...
func beginImmediateQueueTransaction(ctx context.Context, db *sql.DB) (*sql.Tx, error) {
	...
	var lastBusy error
	for {
		if lastBusy != nil && time.Until(deadline) <= 0 {
			return nil, lastBusy
		}
		...
		if !isQueueSQLiteBusy(err) {
			return nil, err
		}
		lastBusy = err

		delay := min(queueBusyRetryInterval, time.Until(deadline))
		if delay <= 0 {
			return nil, lastBusy
		}
		if err := waitForQueueBusyRetry(ctx, delay); err != nil {
			return nil, err
		}
	}
}

func waitForQueueBusyRetry(ctx context.Context, delay time.Duration) error {
...
func isQueueSQLiteBusy(err error) bool {
	sqliteErr, ok := errors.AsType[queueSQLiteCodeError](err)
	return ok && sqliteErr.Code()&0xff == queueSQLiteBusyPrimaryCode
}

func initializeMessageQueueSchema(ctx context.Context, db *sql.DB) (err error) {
	tx, err := beginImmediateQueueTransaction(ctx, db)
	if err != nil {
...
func verifyQueueConnection(ctx context.Context, tx *sql.Tx) error {
	...
	expectedBusyTimeout := int(queueBusyTimeout.Milliseconds())
	if busyTimeout != expectedBusyTimeout {
		return queueSchemaConflictf(
			"queue busy timeout is %d ms, expected %d ms",
			busyTimeout,
			expectedBusyTimeout,
		)
	}
