...
**CHOOK-03** When a trusted hook command or any executable asset it invokes, sources, passes as an interpreter operand, embeds as inline interpreter code, consumes as runtime code through an interpreter or command-executing pipeline mediator, loads as an external jq filter, module, test, or file-backed argument, reconstructs and launches through a command-capable runtime such as AWK, sed, `find -exec`, a command-executing `tar` option, or Git outside its explicit safe subcommand/option set, resolves as a command word or command-wrapper target through shell expansion, invokes through a string-evaluating shell builtin such as `eval` or `trap`, or writes variables through an assignment-capable arithmetic expression references a mutable project, working, home, temporary, relative, or non-system absolute runtime path instead of a committed asset through `AGM_CODEX_HOOK_ROOT`, mutates the hardened `PATH`, assigns a dynamic-loader, shell-startup, interpreter, or Git environment variable that can load or execute unverified runtime code, references a missing or uncommitted asset, or can be shadowed by a nested unattested hook manifest between the launch directory and repository root, the system shall reject the attestation; every materialized-root dependency shall be recursively pinned, and commands for events that are replaced with an OS-owned no-op before bypassed launch are excluded from runtime-asset attestation.
...
**CHOOK-17** When the governed AGM build enables unattended projection of the terminal SPEC adapter, it shall first build the helper with ambient Go workspace mode disabled, embed that exact artifact's SHA-256 into AGM, and the private executor shall reject a missing, malformed, stale, replaced, or otherwise untrusted deployed helper before generating the session command; an unstamped standalone AGM build shall fail closed for this projection.
