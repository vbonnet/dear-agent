...
func TestTrustedHookAssetsRejectDynamicCommandResolution(t *testing.T) {
	for _, script := range []string{
		...
		"#!/bin/sh\nAWKPATH=. /usr/bin/gawk -i helper.awk 'BEGIN { print \"main\" }'\n",
		...
		"#!/bin/sh\n/usr/bin/xargs --eof ./helper </dev/null\n",
		"#!/bin/sh\n/usr/bin/xargs --replace ./helper </dev/null\n",
		"#!/bin/sh\n/usr/bin/xargs --max-lines ./helper </dev/null\n",
		...
		"#!/bin/bash\nexec -a hook-shell /bin/bash helper\n",
		"#!/bin/bash\ncommand exec -a hook-shell /bin/bash helper\n",
		"#!/bin/sh\n/usr/bin/env -u SAFE /bin/bash helper\n",
		"#!/bin/sh\ncommand /usr/bin/env --unset SAFE /bin/bash helper\n",
		"#!/bin/sh\n/usr/bin/env -P /usr/bin /bin/bash helper\n",
		"#!/bin/sh\n/usr/bin/taskset -c 0 /bin/bash helper\n",
		"#!/bin/sh\n/usr/bin/unshare /bin/bash helper\n",
		"#!/bin/sh\n/usr/bin/perl -I. -Mhelper /usr/bin/true\n",
		"#!/bin/sh\n/usr/bin/perl -wI. -wMhelper /usr/bin/true\n",
		...
		"#!/bin/sh\n/usr/bin/gawk -W exec=helper.awk 'BEGIN { print \"main\" }'\n",
		"#!/bin/sh\n/usr/bin/gawk -Wexec=helper.awk 'BEGIN { print \"main\" }'\n",
		"#!/bin/sh\n/usr/bin/gawk -e 'BEGIN { system(\"./helper\") }' /dev/null\n",
		"#!/bin/sh\n/usr/bin/gawk -e'BEGIN { system(\"./helper\") }' /dev/null\n",
		"#!/bin/sh\n/usr/bin/gawk --source='BEGIN { system(\"./helper\") }' /dev/null\n",
		"#!/bin/sh\n/usr/bin/gawk --sou 'BEGIN { system(\"./helper\") }' /dev/null\n",
		...
		"#!/bin/sh\n/usr/bin/seq 1000 -1 1 | /usr/bin/sort --buffer-size=1K --compress-program='.''/'helper\n",
		"#!/bin/sh\n/usr/bin/sort --compress-prog=./helper < /dev/null\n",
		"#!/bin/sh\n/usr/bin/sort \"$SORT_OPTIONS\" < /dev/null\n",
...
func TestTrustedHookAssetsAllowLiteralCommandsAndExpandedArguments(t *testing.T) {
	for _, script := range []string{
		...
		"#!/bin/sh\n/usr/bin/sort --buffer-size=1K < /dev/null\n",
		"#!/bin/sh\n/usr/bin/python3 -I /usr/bin/true\n",
