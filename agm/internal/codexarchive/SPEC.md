...
**CAX-04** When a persisted Codex thread id is archived, the archive bridge shall first invoke the public `codex archive --remote unix:// <thread-id>` command, unless `AGM_CODEX_REMOTE` explicitly overrides that endpoint; it shall not use the app-server archive request.
...
**CAX-08** When remote archival of a persisted Codex session id fails while the caller context remains active, the archive bridge shall retry the same id through the public local saved-session `codex archive <thread-id>` command so imported and local-only sessions do not depend on Remote Control.
