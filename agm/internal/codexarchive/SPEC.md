...
**CAX-04** When Codex app-server archival fails or its remote endpoint is unavailable for a persisted Codex thread id, the archive bridge shall fall back to the public Codex CLI archive command through that session's `unix://` endpoint, unless `AGM_CODEX_REMOTE` explicitly overrides it.
