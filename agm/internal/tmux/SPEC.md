...
**TMUX-32** When pre-input AGY readiness encounters an active first-run color-theme or Terms of Service/Data Use screen, the system shall stop promptly with explicit interactive-onboarding guidance and shall not send keys that accept preferences, legal terms, or data-use choices on the operator's behalf; create and resume callers shall propagate this failure before prompt delivery or attachment. Detection shall tolerate pane line wrapping, while post-input transcript text and onboarding markers present only before the latest composer shall not be classified as an active onboarding screen.
