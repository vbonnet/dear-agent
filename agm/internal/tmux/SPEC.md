...
**TMUX-48** When Codex startup shows either the structured selector for new or changed executable hooks or the active hooks dashboard with review-required hooks, every direct and shared readiness wait shall inspect the complete captured screen, stop promptly with a typed, actionable error that requires interactive operator review, classify the pane as review-required rather than ready or generically busy, and not send keys, trust hooks, register the session, deliver a startup prompt, attach, or update lifecycle metadata; trailing blank terminal rows shall not hide an active review surface, an active dashboard redraw below an older composer shall remain authoritative, and retained selector or dashboard text followed by a newer tail-owned composer shall not remain a blocker.

**TMUX-49** When Codex renders its current welcome view with instructions between the composer header and cursor, the system shall preserve terminal styles and classify the structured terminal footer paired with dim or grey suggestion text as an empty composer while treating identical unstyled text as a human draft; direct startup and generic prompt waits, idle probes, shared readiness, session and backend state detection, and prompt delivery shall use the same distinction.
