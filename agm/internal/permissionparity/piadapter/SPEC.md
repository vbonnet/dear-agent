...
**PI-AUTH-07** When repository lifecycle hooks execute, the extension shall project shared hook metadata and shall enforce structured block results by consuming rejected user input, canceling rejected compaction, blocking rejected tools, or delivering bounded Stop feedback instead of relying only on process exit status.
