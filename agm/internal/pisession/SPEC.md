...
**PI-SESSION-09** When a Pi coding-agent directory is supplied for create, import, or cold resume, the system shall normalize it to an absolute path, require an existing non-symlink directory, and preserve the validated path as native session metadata; create and import shall also persist field presence when no directory is supplied so Pi's native default remains intentional, while metadata that genuinely predates that marker shall retain the invoking environment fallback for compatibility.
