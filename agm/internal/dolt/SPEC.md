...
**DOLTR-06** When a resume transaction provisionally changes a tmux session name, the storage adapter shall use an opaque cross-dialect ownership revision to compare-and-swap only the name, preserve unrelated columns, restore only that same provisional revision and its prior activity timestamp after failure, and rotate the revision after success so a newer concurrent update is never overwritten. If commit reports an error, the adapter shall re-read the exact prior and provisional revisions; it shall permit tmux cleanup without a pending metadata change only when the complete prior state proves the write did not commit, and otherwise shall preserve the provisional change for compare-and-swap compensation. A full-session update shall carry the revision observed by its read; it shall change the tmux name only if that revision is current, shall otherwise preserve the current tmux name while applying unrelated fields, and shall always advance the revision so any number of older snapshots remain unable to reopen the tmux-name compare-and-swap. An authoritative session rename shall atomically compare the prior user-visible name, tmux name, and observed revision before changing both names and advancing a generated revision; after an autocommit error it shall re-read that exact revision without caller cancellation, treat the intended names under a later revision as superseded success, and authorize tmux rollback only when both prior names are still current. Superseded rollback shall preserve the canonical tmux session without relying on timestamp precision.
