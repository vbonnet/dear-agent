...
**DOLTR-06** When a resume transaction provisionally changes a tmux session name, the storage adapter shall use an opaque cross-dialect ownership revision to compare-and-swap only the name, preserve unrelated columns, restore only that same provisional revision and its prior activity timestamp after failure, and release the revision after success so a newer concurrent update is never overwritten. If commit reports an error, the adapter shall re-read the exact prior and provisional revisions; it shall permit tmux cleanup without a pending metadata change only when the complete prior state proves the write did not commit, and otherwise shall preserve the provisional change for compare-and-swap compensation. A full-session update shall carry the revision observed by its read and shall preserve a different current tmux name and ownership revision while still applying unrelated fields, so a stale snapshot cannot orphan the provisionally owned session.
