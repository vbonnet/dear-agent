...
**AGM-DOLT-MIGRATION-03** When migration 018 is applied, the system shall add a nullable `tmux_session_revision` column without changing existing session rows, so MySQL/Dolt and isolated SQLite stores can use the same opaque resume-ownership token semantics.
