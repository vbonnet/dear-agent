...
**AGP-21** When the AGY adapter creates or cold-resumes a session, the system shall use the shared canonical AGY command builder, preserve the selected model, permission mode, authorized directories, native conversation ID, quoting, and process-exit policy, and require native readiness before returning success or attaching; when an imported conversation has no defensible model provenance, cold resume shall omit `--model` so AGY retains the saved native selection.
