...
func TestBuildPiCommandSafelyForwardsOptionalCodingAgentDirectory(t *testing.T) {
	...
	if !strings.Contains(configured, `PI_CODING_AGENT_DIR='/tmp/pi agent'"'"'s config' pi`) {
	...
	if strings.Contains(defaulted, "PI_CODING_AGENT_DIR") {
		t.Fatalf("default command overrode Pi configuration discovery: %s", defaulted)
	}
}
