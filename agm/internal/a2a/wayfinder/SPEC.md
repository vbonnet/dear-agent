...
`agm/internal/a2a/wayfinder` converts Wayfinder status, phase transitions,
task updates, handoffs, and blockers into canonical A2A protocol messages.

## EARS Requirements

**A2A-WF-01** When Wayfinder status is converted to an A2A message, the system shall include session ID, current phase, status, phase progress, and next steps.

**A2A-WF-02** When Wayfinder status is completed, the system shall mark the A2A message as consensus reached.

**A2A-WF-03** When a phase transition is converted, the system shall produce an awaiting-response message that asks for approval to proceed to the new phase.
