...
Feature: Harness parity
  ...
  Scenario: Failed Codex resume is rolled back before success effects
    ...
    And authoritative session renames should fence ambiguous storage writes, preserve claimed tmux identity across lost replies and server restarts, reject stale identity revisions, and compensate tmux after storage conflicts
