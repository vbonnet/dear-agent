...
Feature: Harness parity
  ...
  Scenario: Failed Codex resume is rolled back before success effects
    ...
    And a failed Codex resume should serialize concurrent attempts, compensate owned provisional metadata before removing its creation-specific tmux identity, and preserve tmux whenever metadata cleanup is unproven
