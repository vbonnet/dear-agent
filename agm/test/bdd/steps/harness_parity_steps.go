...
func RegisterHarnessParitySteps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^API delivery should restore persisted configuration, linearize archive with bounded completed turns, renew fan-out deadlines, require adapter readiness without tmux, and document its compatibility-only control plane$`, singleAndFanOutAPISendsShouldUseAdapterSessionReadinessWithoutRequiringTmux)
