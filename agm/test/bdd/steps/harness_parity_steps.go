...
func RegisterHarnessParitySteps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^API delivery should restore persisted configuration, linearize archive and deletion with bounded completed turns, renew fan-out deadlines, honor request cancellation during reconstruction and readiness, require adapter readiness without tmux, and document its compatibility-only control plane$`, singleAndFanOutAPISendsShouldUseAdapterSessionReadinessWithoutRequiringTmux)
