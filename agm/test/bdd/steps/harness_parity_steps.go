...
func RegisterHarnessParitySteps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^API delivery should restore persisted configuration, linearize archive and deletion with bounded completed turns, renew fan-out deadlines with provider headroom, honor request cancellation during reconstruction and readiness, preserve large JSONL records, batch imports, require adapter readiness without tmux, and document its compatibility-only control plane$`, singleAndFanOutAPISendsShouldUseAdapterSessionReadinessWithoutRequiringTmux)
