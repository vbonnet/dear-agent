...
func RegisterHarnessParitySteps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^API delivery should restore persisted configuration, linearize archive with bounded completed turns, renew fan-out deadlines, and require adapter readiness without tmux$`, singleAndFanOutAPISendsShouldUseAdapterSessionReadinessWithoutRequiringTmux)
