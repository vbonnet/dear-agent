...
func RegisterHarnessParitySteps(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^forced CLI message sends should replace only positively identified queued AGM input$`, forcedCLIMessageSendsShouldReplaceOnlyPositivelyIdentifiedQueuedAGMInput)
	...
	ctx.Step(`^API delivery should restore persisted configuration, linearize archive with bounded completed turns, and require adapter readiness without tmux$`, singleAndFanOutAPISendsShouldUseAdapterSessionReadinessWithoutRequiringTmux)
