...
**MCS-15** When the compiled tool surface is audited, the system shall enumerate the production SDK registration through an in-memory MCP client and reject every registry, schema, mapping, or discovery difference not consumed by one exact compatibility record.

**MCS-16** When archive or kill handlers invoke a shared mutation operation, the system shall propagate the MCP request context separately from input-to-request adaptation.
