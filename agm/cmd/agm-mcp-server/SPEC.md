...
**MCS-15** When the compiled tool surface is audited, the system shall enumerate the production SDK registration through an in-memory MCP client, preserve exact server-side schema values before generic client decoding, and reject every registry, complete property-constraint or nested array-item schema, mapping, or discovery difference not consumed by one exact compatibility record, including absent versus explicitly empty enum keywords and lossless enum member boundaries.
