...
**MCS-15** When the compiled tool surface is audited, the system shall enumerate the production SDK registration through an in-memory MCP client and reject every registry, schema, mapping, or discovery difference not consumed by one exact compatibility record, including absent versus explicitly empty enum keywords.
