...
// run contains the core logic of the userpromptsubmit-state-reporter hook.
...
func run() bool {
	...
	// Update state to THINKING via agm CLI
	cmd := buildAgmCommand(sessionName, "THINKING", "userpromptsubmit-hook")
	_ = cmd.Run() // Ignore errors - hooks are advisory
	return true
}
