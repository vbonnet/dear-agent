...
func TestBuildAgmCommand_ArgsCorrect(t *testing.T) {
	cmd := buildAgmCommand("test-session", "THINKING", "userpromptsubmit-hook")

	require.NotNil(t, cmd)
	expected := []string{"agm", "session", "state", "set", "test-session", "THINKING", "--source", "userpromptsubmit-hook"}
...
func TestBuildAgmCommand_StdoutAndStderrToStderr(t *testing.T) {
	cmd := buildAgmCommand("sess", "THINKING", "userpromptsubmit-hook")
...
func TestBuildAgmCommand_SpecialCharactersInSession(t *testing.T) {
	cmd := buildAgmCommand("session/with/slashes", "THINKING", "userpromptsubmit-hook")
	assert.Equal(t, "session/with/slashes", cmd.Args[4])
}

func TestBuildAgmCommand_EmptySessionName(t *testing.T) {
	cmd := buildAgmCommand("", "THINKING", "userpromptsubmit-hook")
	require.NotNil(t, cmd)
...
func TestIntegration_EnvToCommand(t *testing.T) {
	...
	cmd := buildAgmCommand(name, "THINKING", "userpromptsubmit-hook")
	assert.Equal(t, []string{"agm", "session", "state", "set",
		"integration-test", "THINKING", "--source", "userpromptsubmit-hook"}, cmd.Args)
}
