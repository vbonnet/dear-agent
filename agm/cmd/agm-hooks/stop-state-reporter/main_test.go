...
func TestBuildAgmCommand_ArgsCorrect(t *testing.T) {
	cmd := buildAgmCommand("test-session", "READY", "stop-hook")

	require.NotNil(t, cmd)
	expected := []string{"agm", "session", "state", "set", "test-session", "READY", "--source", "stop-hook"}
...
func TestBuildAgmCommand_StdoutAndStderrToStderr(t *testing.T) {
	cmd := buildAgmCommand("sess", "READY", "stop-hook")
...
func TestIntegration_EnvToCommand(t *testing.T) {
	...
	cmd := buildAgmCommand(name, "READY", "stop-hook")
	assert.Equal(t, []string{"agm", "session", "state", "set",
		"integration-test", "READY", "--source", "stop-hook"}, cmd.Args)
}
