...
**AGM-CMD-HOOK-03** When Claude emits SessionStart for an AGM-launched session, the installed hook shall preserve the payload's conversation UUID, retry association asynchronously for at least the complete 90-second launch-readiness window when detached creation has not registered the AGM session yet, and report the session READY only after association succeeds, including when the Claude command was queued behind current-pane AGM creation.
