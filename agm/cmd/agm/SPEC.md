...
- **Behavior:**
  ...
  - **Force/autonomous recovery:** Bypass preliminary queue routing only so the shared atomic operation can replace a current queued-input artifact bearing a complete AGM message header; never replace a human draft or bypass generic busy, active-work, permission, overlay, onboarding, wrong-harness, missing-target, or backend-error states
  - **State-based routing:**
