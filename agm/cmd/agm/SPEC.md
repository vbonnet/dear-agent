...
- **Behavior:**
  ...
  - **Force/autonomous recovery:** For manifest-confirmed supported CLI harnesses only, bypass preliminary queue routing so the shared atomic operation can replace the latest queued-input artifact when a syntactically complete generated AGM message header is observably bound inside that same current composer; API harnesses deliver directly only from preliminary `YES`, recheck that state at the final common single-recipient or fan-out delivery boundary, and reject every non-ready state because the daemon queue is tmux-only, and no path may infer AGM ownership from a historical or partial header or an opaque Codex pasted-content chip, replace a human draft, or bypass generic busy, active-work, permission, overlay, onboarding, wrong-harness, missing-target, or backend-error states
  - **State-based routing:**
