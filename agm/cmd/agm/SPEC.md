...
**CLI-36** When `agm session rename` moves a live tmux session, the system shall persist the user-visible and tmux names together through a narrow compare-and-swap against the exact identity revision read before the move. If another writer advanced that identity, the command shall report a conflict, restore the live tmux session's prior name, join any rollback failure with the storage conflict, and never report rename success with metadata pointing at a nonexistent tmux name.
