...
**CLI-35** If AGM creates a tmux session for a resume attempt and creation finalization, command dispatch, harness readiness, ordinary or caller-canceled prompt delivery, or canonical-name persistence fails, the system shall compensate the exact immutable tmux session identity created by that attempt and return the primary failure together with any cleanup failure. After successful readiness, canonical-name persistence shall compare-and-swap only the live identity before optional Codex prompt submission; a later prompt failure shall restore that exact provisional metadata revision without overwriting concurrent session updates. A same-named replacement tmux session or a tmux session that existed before the attempt shall not be removed.
