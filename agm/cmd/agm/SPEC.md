...
**CLI-36** When `agm session rename` moves a live tmux session, the system shall persist the user-visible and tmux names together through a narrow compare-and-swap against the exact identity revision read before the move. If another writer advanced that identity or the caller is canceled after the move, the command shall report the primary failure, restore the live tmux session's prior name with a bounded cancellation-independent context, join any rollback failure, and never report rename success with metadata pointing at a nonexistent tmux name.
