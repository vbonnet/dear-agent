...
- **Behavior:**
  ...
  - **Force/autonomous recovery:** For manifest-confirmed supported CLI harnesses only, bypass preliminary queue routing so the shared atomic operation can replace the latest queued-input artifact when a syntactically complete generated AGM message header is observably bound inside that same current composer; API harnesses preserve their preliminary delivery state and reject when the required queue is unavailable rather than falling back to unverified direct delivery, and no path may infer AGM ownership from a historical or partial header or an opaque Codex pasted-content chip, replace a human draft, or bypass generic busy, active-work, permission, overlay, onboarding, wrong-harness, missing-target, or backend-error states
  - **State-based routing:**
