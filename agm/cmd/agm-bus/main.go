...
func cmdServe(args []string) error {
	...
	matrixUserID := fs.String("matrix-user-id", "", "Matrix bot user id, e.g. @agmbus:example.org (default: MATRIX_USER_ID env var)")
	matrixRoomID := fs.String("matrix-room", "", "Matrix room id the adapter listens to and posts to (default: MATRIX_ROOM_ID env var)")
	matrixAllowlist := fs.String("matrix-allowlist", "", "comma-separated Matrix user ids (mxids) allowed to send")
	...
	// Discord but against a Matrix homeserver; supports Google Chat via
	// mautrix-googlechat bridging since bridged users appear as regular
	// Matrix users in the configured room.
	if *matrixEnabled {
		...
		default:
			var allowlist []string
			if *matrixAllowlist != "" {
				for _, mxid := range strings.Split(*matrixAllowlist, ",") {
					mxid = strings.TrimSpace(mxid)
					if mxid != "" {
						allowlist = append(allowlist, mxid)
					}
				}
			}
