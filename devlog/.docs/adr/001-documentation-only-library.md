...
**Related Decisions**:
- Relationship to `claude-session-manager` (implements session management)
...
Documentation-only creates clear differentiation from tools like `claude-session-manager` and `engram`.
...
- `claude-session-manager` can automate workspace setup
...
- Overlaps with `claude-session-manager`
...
- `claude-session-manager`: Session management automation
