...
**Repositories migrated**: engram, vida, [REDACTED_EMPLOYER]-health-iac, grouper, aegis, ai-tools, beads, velvet, vpaste
...
## Example 2: vida (Convert Migration)
...
git clone --bare ~/repos/vida/base/.git ~/repos/vida/.bare

# 2. Configure remote fetch
git -C ~/repos/vida/.bare config remote.origin.fetch '+refs/heads/*:refs/remotes/origin/*'
...
rm -rf ~/repos/vida/base

# 5. Recreate base as worktree
git -C ~/repos/vida/.bare worktree add ~/repos/vida/base $BASE_BRANCH
...
## Example 3: [REDACTED_EMPLOYER]-health-iac (Transform Migration)

### Before Migration

```
[REDACTED_EMPLOYER]-health-iac/
...
```bash
# 1. Clone .git as bare
git clone --bare ~/repos/[REDACTED_EMPLOYER]-health-iac/.git ~/repos/[REDACTED_EMPLOYER]-health-iac/.bare

# 2. Configure remote fetch
git -C ~/repos/[REDACTED_EMPLOYER]-health-iac/.bare config remote.origin.fetch '+refs/heads/*:refs/remotes/origin/*'

# 3. Get current branch
cd ~/repos/[REDACTED_EMPLOYER]-health-iac
...
git -C .bare worktree add ~/repos/[REDACTED_EMPLOYER]-health-iac/main $CURRENT_BRANCH
...
# (Same as [REDACTED_EMPLOYER]-health-iac example)
```
...
| vida | Convert | base/.git → | .bare/ + recreate base/ | ⭐⭐ Medium |
| [REDACTED_EMPLOYER]-health-iac | Transform | .git → | .bare/ + main/ | ⭐⭐⭐ Complex |
