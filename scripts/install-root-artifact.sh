...
/bin/mv -f "$staging" "$destination"; trusted_file "$destination" || exit 2; test "$(file_identity "$destination")" = "$staged_identity" || exit 2; activated_hash=$(/usr/bin/openssl dgst -sha256 -r "$destination"); activated_hash=${activated_hash%% *}; test "$activated_hash" = "$expected_hash"
