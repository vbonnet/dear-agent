...
if [ -z "$last" ] || printf %s "$last" | grep -q '[^0-9]'; then reason="heartbeat is missing or invalid: $heartbeat"
elif [ "$last" -gt "$now" ] || [ $((now-last)) -gt "$max_age" ]; then reason="heartbeat is stale: $heartbeat"; fi
[ -z "$reason" ] && exit 0
