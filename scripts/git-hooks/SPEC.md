...
**GITHOOK-16** When build-relevant AGM source changes, the system shall serialize deployment across checkouts sharing the install directory, stage both `agm` and the separately installed `agm-reaper` from the same resolved trunk revision, preserve the installed pair if either build fails, and activate the pair only after both builds succeed.
