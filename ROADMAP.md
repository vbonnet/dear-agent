...
**X1. Bounded bot-review loop.** Delta-scoped re-review after round 1, severity floor
on required conversation resolution, round budget of 4 with human escalation, land or
de-require the keyless gate (#1221). Must not regress to ce-lr7j severity-blind
auto-resolve. Evidence: #1096 died after 28 rounds/51 threads; #1243 hit 76 threads
in under 2 days; dominant per-PR latency (retro 2026-08-18-unbounded-bot-review-loop;
retro 2026-08-11-dear-agent-pr-merge-latency).
**X2. Remediation lane through the guard stack.** Config-guard JSON parsing fix
...
**X3. Quota meter and tightest-window accounting.** ce-3rmqx.1/.2/.3; #1197, #1218,
...
**X4. Trustworthy local preflight.** Skip the macOS-broken WRONG_HARNESS test locally
...
**X5. Claimed-vs-verified gates.** ce-hj7eb.1 (receipts required, machine-checked)
...
**X6. Reserved supervisor capacity, load shedding, starvation dashboard.** Factorio
...
**X7. Trace-mining flywheel, first increment, now explicitly gated.** ce-asrxq.1
after ce-9xotf.3 (dossier schema and prevalence stats) and ce-qf0f (schedule the
dormant sensors; signals.db is empty). Encode those as real Beads dependencies; today
the epic shows READY while its own description names unmet prerequisites. Evidence:
bd dep tree shows no blockers; the miner would consume exactly the session data N1
and N3 currently make unreliable (beads ce-asrxq; PR #322 cluster notes).
**X8. Dreaming: manual dream run plus memory CAS.** ce-9xotf.2 (one manual run
...
**X9. Counterfactual replay routing, first increment.** ce-qrwt3.1, 20 decision
points, 2 candidate models, sharing one normalized trace digest with X7. After X3,
because substitution decisions need real cost data (bead ce-qrwt3; dreaming plan 3c).
...
- Research-epic freeze from the 07-26 review (ce-okcw.*, ce-kh70.*, ce-lafa.*):
  retained, with two carve-outs already priced in: X8 (dream run) and design work
...
2. Top theme. Was: flywheel wiring first (traces to retros to evals). Now:
   ...
   it (X7-X9). Evidence: week of 08-11..18: 12/32 spawns wedged, 56 OAuth family
...
3. Trace-mining epics (ce-asrxq, ce-qrwt3, ce-nsuo8). Were: fresh Fable designs
   (08-14), implicitly next up. Now: X7/X9 gated on sensors, schema, and truthful
...
7. Truth reconciliation (07-26 CP-2). Was: CP-2. Now: split into N3 (pipeline
   truth) and X5 (claim truth), in that order. Evidence: the 08-11 twin retros show
   they are distinct failures that compound.
8. Factorio shortlist (PR #321). Was: standalone 8-item research shortlist. Now:
   folded in as X6 and X10; items overlapping friction work (breakers, hysteresis,
...
10. Dreaming plan (08-17). Was: new research, unranked. Now: X8 admitted ahead of
