...
.PHONY: lint-doc-freshness lint-links
...
# Parse every tracked Markdown file, including hidden paths. Existing debt is
# explicit and shrink-only; new findings and stale exceptions both fail.
lint-links:
	@go run ./tools/dead-links --root . --baseline .dead-links-baseline.txt
