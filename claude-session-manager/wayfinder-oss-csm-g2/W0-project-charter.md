...
- High confidence that `csm new --agent=gemini` works as well as `csm new --agent=claude`
...
4. Run tests against both `--agent=claude` and `--agent=gemini`
...
- [ ] Tests run successfully for both `--agent=claude` and `--agent=gemini`
...
**Dependencies**:
- oss-csm-g1 (Gemini implementation) must be complete
