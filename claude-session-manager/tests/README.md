...
- `tmux` installed and accessible
- `claude` CLI command available
- `csm` binary built and in PATH (`~/go/bin/csm`)
...
✓ csm found: csm version 2.0.0-dev
...
✓ Archived session appears in 'csm list --all'
✓ Archived session hidden from 'csm list' (correct)
✓ csm get-uuid works for archived session
