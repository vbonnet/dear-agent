...
This document compares behavioral differences between Claude, Gemini, and GPT mock adapters in the claude-session-manager BDD test suite. These mock adapters simulate agent behavior deterministically for testing purposes without requiring real API calls.
...
| Error Detail | ⚠️ Basic | ⚠️ Basic | ✅ Enhanced | GPT includes actionable guidance in error messages | GPT: `"session X not found. Verify with 'csm list'."` |
...
| Invalid session ID | Claude, Gemini, GPT | Call `SendMessage` with non-existent session ID | Error: `"session {id} not found"` (GPT adds: `"Verify with 'csm list'."`) | `session_lifecycle.feature:49-59` |
...
| Archived session messaging | Claude, Gemini, GPT | Archive session, attempt to send message | Error: `"session {id} is archived"` (GPT adds: `"Use 'csm resume' to reactivate."`) | Existing scenarios (implicit in archive tests) |
...
- **Mock Adapters**: `~/src/ws/oss/repos/ai-tools/main/claude-session-manager/test/bdd/internal/adapters/mock/`
- **BDD Scenarios**: `~/src/ws/oss/repos/ai-tools/main/claude-session-manager/test/bdd/features/`
- **Test Execution**: `go test ./test/bdd/... -v`

---

**Last Updated**: 2026-01-18
**Maintained By**: claude-session-manager development team
