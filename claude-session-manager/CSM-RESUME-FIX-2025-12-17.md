...
**Problem**:
`make install` only installed to `~/.local/bin/`, but `~/go/bin/csm` was in PATH first and contained an outdated binary.
...
install: build
	mkdir -p ~/.local/bin
	mkdir -p ~/go/bin
	cp bin/csm ~/.local/bin/csm
	cp bin/csm ~/go/bin/csm
	@echo "✓ csm binary installed to ~/.local/bin/csm and ~/go/bin/csm"
```
...
```bash
csm associate <session-name>           # Auto-detect from history
csm associate <session-name> --uuid <uuid>  # Explicit UUID
```
