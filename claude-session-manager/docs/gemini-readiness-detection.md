...
This document describes the robust readiness detection implementation for Google Gemini CLI in csm-agent-wrapper, following the same pattern used for Claude.

## Problem Statement

Previously, csm-agent-wrapper used a simple 500ms delay when starting Gemini, which was:
...
### Integration in csm-agent-wrapper
...
### Integration Testing
1. Start Gemini session: `csm-agent-wrapper --agent=gemini test-session`
2. Check debug logs: `CSM_DEBUG=1 csm-agent-wrapper --agent=gemini test-session`
