...
**Issue:**
- `csm new` and `csm resume` held the global lock for the entire duration of tmux session attachment
...
- Any other `csm` command would fail with "Another csm command is currently running"
...
**Solution:**
- Added `csm unlock` command with PID validation
...
- Updated error message to suggest `csm unlock` instead of `--no-lock`
...
**Before:**
```
csm new → PersistentPreRunE (lock) → RunE (create tmux, start Claude, ATTACH) → PersistentPostRunE (unlock)
          ^                                                                                      ^
          |                                                                                      |
...
**After:**
```
csm new → PersistentPreRunE (lock) → RunE (create tmux, start Claude, UNLOCK, attach) → PersistentPostRunE (noop)
          ^                                                          ^
          |                                                          |
...
Manual test script: `~/src/ws/test-csm-lock-fixes.sh`

Tests:
1. Lock-free commands work without locks
2. `csm unlock` detects no lock when clean
3. `csm unlock` removes stale locks
4. `csm version` doesn't create locks
...
```bash
# Terminal 1
csm new test-session-1
# (stay attached)

# Terminal 2 - should work now!
csm new test-session-2
```

**Stale Lock Recovery:**
```bash
# Simulate crash
echo "99999" > /tmp/csm-$(id -u)/csm.lock
...
- ❌ Agents manually deleting lock files with `rm -rf /tmp/csm-*.lock`
- ❌ `csm new` in one terminal blocked all other csm commands
- ❌ `csm version` required `--no-lock` flag to bypass locks
- ❌ No safe way to remove stale locks after crashes

### After

- ✅ Safe `csm unlock` command with PID validation
