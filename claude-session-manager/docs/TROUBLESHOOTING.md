...
```bash
# ❌ Wrong
agm new my-session --agent cluade
...
2. Create new session: `agm new my-session --agent <agent>`
...
**Solution:**
1. Verify AGM installed: `which agm` or `which csm`
...
agm rename old-name new-name --agent gemini
...
3. **If wrong agent:**
   ...
   - Recreate with correct agent: `agm new my-session --agent <correct-agent>`
...
agm new test-session --agent claude --debug
...
4. **File issue:** [GitHub Issues](https://github.com/vbonnet/ai-tools/issues)
...
- AGM version: `agm --version` or `csm --version`
...
**Manifest:** (attached)
**Steps:**
1. Created session: agm new research-session --agent gemini
