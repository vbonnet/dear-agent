...
pipeline {
    ...
    stages {
        stage('Setup AGM') {
            steps {
                sh 'go install github.com/vbonnet/ai-tools/claude-session-manager/cmd/csm@latest'
            }
        }

        stage('Create Session') {
            steps {
                sh 'agm new --agent claude ci-build-${BUILD_NUMBER}'
            }
        }
...
jobs:
  agm-test:
    ...
    steps:
      ...
      - run:
          name: Install AGM
          command: go install github.com/vbonnet/ai-tools/claude-session-manager/cmd/csm@latest
      - run:
          name: Install tmux
...
```bash
# ~/.config/csm/hooks/on-session-create.sh
#!/bin/bash
SESSION=$1
...
# ~/.config/csm/config.yaml
hooks:
  on_session_create: "~/.config/csm/hooks/on-session-create.sh"
  on_session_archive: "~/.config/csm/hooks/on-session-archive.sh"
  on_health_check_fail: "~/.config/csm/hooks/on-health-fail.sh"
```
