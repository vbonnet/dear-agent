...
agm new --agent gpt ideation-session
...
agm new --agent claude backend-api

# Use Gemini for research tasks
agm new --agent gemini market-analysis

# Use GPT for quick chats
agm new --agent gpt daily-standup
```
...
ℹ Scanning sessions directory: /home/user/.claude-sessions
...
```bash
# Code review → Claude
agm new --agent claude pr-review-backend

# Research task → Gemini
agm new --agent gemini competitor-analysis

# Brainstorming → GPT
agm new --agent gpt product-ideas
```
...
agm resume my-session --agent claude
...
agm new old-session --agent claude
...
agm new --agent claude coding-work
...
```bash
agm new --agent claude   code-review-backend
agm new --agent gemini   research-market-trends
agm new --agent gpt      brainstorm-product-ideas
```
