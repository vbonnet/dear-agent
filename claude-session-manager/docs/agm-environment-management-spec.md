...
- Users expect `agm new --agent gemini` to "just work"
...
```
┌─────────────────────────────────────────────────────────┐
│ User runs: agm new --agent gemini research-task        │
└─────────────────────────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────┐
...
4. **Session Creation Gate** (`agm new --agent gemini`)
...
Command Checks:
  ✓ gemini CLI installed (/home/user/.npm-global/bin/gemini)
...
Option 3: Per-session export (temporary)
  ...
  $ agm new --agent gemini my-session
...
3. Session Creation Gate
   - Validate environment in `agm new --agent gemini`
...
- `agm new --agent gemini` blocks invalid environment
...
- [ ] `agm new --agent gemini` validates environment before session creation
...
- **Time to first success**: User runs `agm new --agent gemini` and succeeds (target: <5 minutes)
...
- **Adoption rate**: % of AGM users who use `--agent gemini` (target: >30% within 1 month)
...
- Agent selection mechanism (`agm new --agent <agent>`)
