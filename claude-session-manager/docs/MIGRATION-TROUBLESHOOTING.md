...
python3 <<EOF
import yaml
with open('/home/user/.claude-sessions/my-session/manifest.yaml') as f:
    try:
        yaml.safe_load(f)
        print("YAML is valid")
...
# Create new session
agm new my-session --agent claude --project ~/src/my-project
...
```bash
# Create test session
agm new --agent claude test-api-key
...
**Symptom:**
```
✗ Sessions directory not found: /home/user/.claude-sessions
```
...
lifecycle: ""
context:
  project: "/home/user/src/my-project"
