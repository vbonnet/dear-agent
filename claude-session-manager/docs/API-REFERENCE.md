...
import (
    ...
    "github.com/vbonnet/ai-tools/claude-session-manager/internal/command"
...
func main() {
    manager := session.NewSessionManager("/home/user/sessions")
...
func main() {
    ...
    err := writer.Write("/home/user/sessions/my-session", m)
    if err != nil {
        panic(err)
    }
    ...
    loaded, err := reader.Read("/home/user/sessions/my-session")
    if err != nil {
        panic(err)
    }
...
func main() {
    ...
    if !exists {
        // Create new session
        err = tmux.NewSession("my-session", "/home/user/projects/myapp", false)
        if err != nil {
            panic(err)
        }
    }
...
func main() {
    ...
    entry := &messages.MessageLogEntry{
        MessageID: "1738612345678-csm-send-001",
        Timestamp: time.Now(),
        Sender:    "csm-send",
...
const (
    // Debug logging
    EnvDebug = "CSM_DEBUG" // "true" or "1"

    // Accessibility
    EnvNoColor       = "NO_COLOR"         // "1"
    EnvScreenReader  = "CSM_SCREEN_READER" // "true" or "1"
    ...
    // Configuration
    EnvConfigPath = "CSM_CONFIG" // Path to config.yaml
)
```
...
import (
    "testing"
    "github.com/vbonnet/ai-tools/claude-session-manager/internal/testing"
)
