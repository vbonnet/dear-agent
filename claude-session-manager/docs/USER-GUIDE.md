...
agm unarchive *[REDACTED_EMPLOYER]*          # Matches all with "[REDACTED_EMPLOYER]"
...
```bash
# Claude (default)
agm new --agent claude coding-session
...
**Current:** Manual selection required (`--agent` flag)
...
agm new --agent gemini research-approach
...
ui:
  theme: "csm"                     # UI theme (csm, dracula, catppuccin)
...
**Available themes:**
- `csm` - High-contrast for dark terminals (default, WCAG AA)
...
**Accessibility:**
- `csm` and `csm-light` are WCAG AA compliant (4.5:1 contrast)
...
agm new design-phase --agent gpt

# Research phase: Use Gemini for large context
agm new research-phase --agent gemini

# Implementation: Use Claude for code
agm new implementation --agent claude
```
...
agm new research-microservices --agent gemini
# ... research multiple papers ...
agm archive research-microservices

# 2. Design with GPT
agm new design-architecture --agent gpt
# ... brainstorm architecture ...
agm archive design-architecture

# 3. Implement with Claude
agm new implement-service --agent claude
# ... write code ...
```
