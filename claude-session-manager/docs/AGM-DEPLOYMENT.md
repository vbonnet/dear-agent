...
wget https://github.com/vbonnet/ai-tools/releases/download/v3.0.0/agm-linux-amd64
...
mkdir -p ~/.config/csm
cat > ~/.config/csm/config.yaml << EOF
defaults:
  interactive: true
...
cd ~/go/src/github.com/vbonnet/ai-tools/claude-session-manager
./scripts/setup-completion.sh
...
```
~/.config/csm/          # Configuration files
...
sudo mkdir -p /etc/csm
sudo cat > /etc/csm/config.yaml.example << EOF
defaults:
  interactive: true
  auto_associate_uuid: true
...
sudo cp scripts/csm-completion.bash /etc/bash_completion.d/csm
```
...
2. System configuration (`/etc/csm/config.yaml`)
3. User configuration (`~/.config/csm/config.yaml`)
