...
hooks:
  SessionStart:
    - name: agm-ready-signal
      command: /home/user/.config/claude/hooks/session-start-agm.sh
```
...
4. **Claude config syntax error**
   ...
   python3 -c "import yaml; yaml.safe_load(open('/home/user/.config/claude/config.yaml'))"
   ```
...
hooks:
  SessionStart:
    - name: my-custom-init
      command: /home/user/.config/claude/hooks/my-init.sh
    - name: agm-ready-signal
      command: /home/user/.config/claude/hooks/session-start-agm.sh
    - name: another-hook
      command: /home/user/.config/claude/hooks/another.sh
```
