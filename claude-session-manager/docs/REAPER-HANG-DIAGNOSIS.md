...
After `/csm-tools:csm-exit` skill completes, the system enters a zero-token waiting loop for 60 seconds before Astrocyte sends ESC to recover. The reaper, which was spawned asynchronously, is also stuck waiting for the Claude prompt to appear.

## Root Cause Analysis

### Incident Timeline (2026-02-02T21:22:24)

1. `/csm-tools:csm-exit` skill executes successfully
...
1. Create test session with `csm test create hang-test`
