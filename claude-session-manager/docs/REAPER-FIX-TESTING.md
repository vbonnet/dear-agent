...
```bash
# Create test session
csm test create idle-test

# Send simple command
csm test send idle-test "echo 'test'"

# Verify prompt detected within 3 seconds (15 idles × 200ms)
time csm test capture idle-test

# Cleanup
csm test cleanup idle-test
```
...
```bash
# Create test session
csm test create multiline-test

# Send command with multi-line output
csm test send multiline-test 'printf "Line 1\nLine 2\nLine 3\n✓ Done\n"'

# Verify prompt detected
time csm test capture multiline-test

# Cleanup
csm test cleanup multiline-test
```
...
```bash
# Create test session
csm test create reaper-test

# Wait for Claude to be ready
sleep 5

# Start reaper in background
csm reaper reaper-test --sessions-dir /tmp/csm-test-reaper-test &
REAPER_PID=$!

# Reaper should wait for prompt, send /exit, archive session
# Monitor reaper process
tail -f /tmp/csm-reaper-reaper-test.log &
...
```bash
# Create test session
csm test create skill-hang-test

# Send multi-line output with emoji (simulates csm-exit skill)
csm test send skill-hang-test 'cat << EOF
...
START=$(date +%s)
csm test capture skill-hang-test
END=$(date +%s)
DURATION=$((END - START))
...
# Cleanup
csm test cleanup skill-hang-test
```
...
Then rebuild:
```bash
go build -C cmd/csm -o ~/go/bin/csm
```
