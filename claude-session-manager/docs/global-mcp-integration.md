...
This document describes the implementation of global MCP integration for AGM (claude-session-manager). The integration allows AGM sessions to detect and connect to global HTTP/SSE MCP servers instead of spawning stdio processes, enabling resource sharing and improved performance.
