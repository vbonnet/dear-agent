...
csm new my-session --agent claude
...
agm new my-session --agent claude   # Claude (code, reasoning)
agm new my-session --agent gemini   # Gemini (research, 1M context)
...
agm new code-work --agent claude      # For coding
agm new research --agent gemini       # For research
agm new chat --agent gpt              # For brainstorming
...
# Code work → Claude
agm new my-code-project --agent claude

# Research → Gemini
agm new my-research --agent gemini
...
agm new my-session --agent claude
agm new my-session --agent gemini
agm new my-session --agent gpt
```
...
agm new --agent gemini research-session  # Use specific agent
agm list --agent claude                  # Filter by agent (if supported)
```
...
**Quick guide:**
- **Code, debugging, reasoning:** `--agent claude`
- **Research, large docs (>200K tokens):** `--agent gemini`
- **Chat, brainstorming, quick Q&A:** `--agent gpt`
...
**Symptom:**
Session created without `--agent` flag uses unexpected agent.

**Solution:**
Always specify `--agent` explicitly:
```bash
# ❌ Unclear which agent
agm new my-session
...
2. Try explicit command: `agm resume <session-name> --agent claude`
...
agm new research --agent gemini  # Gemini for research
```
...
agm new code-work --agent claude
agm new research --agent gemini
agm new chat --agent gpt
```
