...
agm new --agent gemini research-task  # Create with specific agent
...
**Key Behaviors**:
- **Smart defaults**: Auto-selects Claude if no `--agent` specified
...
| Set directory | `/csm-assoc` slash command | Metadata update | Planned |
...
**Commands**:
```bash
agm send my-session "Analyze this code"
agm send my-session --file prompt.txt
agm send my-session --reject          # Reject last response
...
Agent: gemini
Status: Available
CLI: /home/user/.npm-global/bin/gemini
...
# Use Claude for code
agm new --agent claude implement-feature
...
- [ ] `agm new --agent claude my-session` creates Claude session
- [ ] `agm new --agent gemini my-session` creates Gemini session
- [ ] `agm new --agent gpt my-session` creates GPT session (future)
...
- [ ] `agm new --agent gemini` validates environment before creation
- [ ] Clear error messages with fix guidance
