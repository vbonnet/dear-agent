...
- **Claude**: Slash commands (`/rename`, `/csm-assoc`) sent via tmux
...
func (t *ClaudeTranslator) SetDirectory(ctx context.Context, sessionID, dirPath string) error {
    ...
    cmd := fmt.Sprintf("/csm-assoc %s", dirPath)
    return t.tmux.SendKeys(sessionID, cmd)
}
...
| Command | Claude | Gemini | GPT (Future) |
|---------|--------|--------|--------------|
| RenameSession | ✅ Slash command | ✅ API call | 🔜 Planned |
| SetDirectory | ✅ Slash command | ⚠️ Metadata only | 🔜 Planned |
| RunHook | ✅ tmux send | ❌ Not supported | 🔜 Planned |
