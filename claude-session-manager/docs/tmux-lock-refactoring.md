...
These tmux commands update shared server state and can interleave if multiple `csm new` processes run simultaneously.

## Solution

Replaced global command lock with **tmux-scoped lock** (`/tmp/csm-{UID}/tmux-server.lock`):
...
- **InitSequence.Run** (init_sequence.go:30-34) - `/rename` and `/csm-assoc` sequence
...
```
csm resume session-A  → Acquires lock → Blocks for hours (attached to tmux)
csm list              → BLOCKED (waiting for lock)
csm version           → BLOCKED (waiting for lock)
```

### After (Tmux-Scoped Lock)
```
csm resume session-A  → Acquires tmux lock → Releases → Attaches (no lock held)
csm list              → Runs concurrently ✓
csm version           → Runs concurrently ✓
csm new session-B     → Acquires tmux lock briefly → Creates session → Releases
```

### Specific improvements:
1. **Concurrent reads** - Multiple `csm list`, `csm version`, etc. can run simultaneously
2. **No deadlocks** - `/csm-tools:csm-assoc` skill can run without conflict
...
Verified:
- ✅ Build succeeds: `go build ./cmd/csm`
- ✅ Install succeeds: `go install ./cmd/csm`
- ✅ List works: `csm list`
- ✅ Concurrent commands work: `csm list & csm version` (both complete without errors)
