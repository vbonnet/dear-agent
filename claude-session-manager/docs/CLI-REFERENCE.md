...
**Flags:**
```bash
--agent string           Agent to use (claude|gemini|gpt) (default: claude)
...
2. Agent selection (claude/gemini/gpt)
...
- Agent availability (API key configured)
...
--agent string           Filter by agent (claude|gemini|gpt)
...
# Combine filters
agm list --agent gemini --tag research
```
...
agm test send my-test "agm new test-session --agent claude"
```
...
| `2` | Configuration error | Agent not available, API key missing |
