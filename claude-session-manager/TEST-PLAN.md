...
**Commands to Test**:
- [ ] `csm new` - Session creation flow
  - `/rename` sent before `/csm-assoc` (NEW BEHAVIOR)
  - Manifest creation with proper UUID
  - Trust prompt prevention via additionalDirectories
  - Ready-file wait and attach timing
- [ ] `csm resume` - Session attachment
- [ ] `csm list` - Session listing and filtering
- [ ] `csm archive` - Archival with lifecycle state (PARTIALLY TESTED)
- [ ] `csm sync` - UUID population from history
- [ ] `csm status` - Session status reporting
- [ ] `csm doctor` - Health checks
