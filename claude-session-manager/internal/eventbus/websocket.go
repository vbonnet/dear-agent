	...
	for {
		...
		case client := <-h.register:
			...
			if len(h.clients) >= h.maxClients {
				...
				client.conn.Close()
				log.Printf("⚠️  Rejected client connection (max clients: %d)", h.maxClients)
				continue
			}
		...
		case event := <-h.broadcast:
			...
			for client := range h.clients {
				...
				default:
					...
					close(client.send)
					delete(h.clients, client)
					log.Printf("⚠️  Client send buffer full, disconnected")
				}
			}
			h.mu.RUnlock()
...
func (h *Hub) Broadcast(event *Event) {
	...
	default:
		log.Printf("⚠️  Broadcast channel full, event dropped")
	}
}
...
func (c *Client) readPump() {
	...
	for {
		...
		case "subscribe":
			...
			log.Printf("✓ Client subscribed to session: %s", c.sessionFilter)
