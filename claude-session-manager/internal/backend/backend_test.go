...
func TestRegisterDuplicatePanic(t *testing.T) {
	...
	Register("duplicate-backend", factory)
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectError {
				...
				case *TemporalBackend:
					if _, ok := backend.(*TemporalBackend); !ok {
						t.Errorf("expected TemporalBackend, got %T", backend)
					}
				}
			}
		})
...
func TestListBackends(t *testing.T) {
	// Both tmux and temporal should be registered via init()
	backends := ListBackends()

	// We should have at least tmux and temporal
	if len(backends) < 2 {
	...
	if !backendMap["temporal"] {
		t.Error("expected temporal backend to be registered")
	}
}
