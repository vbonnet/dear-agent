		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				if tt.errType != nil && !errors.Is(err, tt.errType) {
					t.Errorf("CapturePaneContent() error = %v, want %v", err, tt.errType)
				}
				return
			}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				if tt.errType != nil && !errors.Is(err, tt.errType) {
					t.Errorf("CapturePaneHistory() error = %v, want %v", err, tt.errType)
				}
				return
			}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				if tt.errType != nil && !errors.Is(err, tt.errType) {
					t.Errorf("GetSessionInfo() error = %v, want %v", err, tt.errType)
				}
				if info != nil {
