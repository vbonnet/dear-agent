...
func TestDetectUUID(t *testing.T) {
	...
	d := NewDetector(historyFile, 5*time.Minute)
...
func TestDetectAndAssociate(t *testing.T) {
	...
	t.Run("auto-apply with high confidence", func(t *testing.T) {
		// Create manifest without UUID
		m := &manifest.Manifest{
			SchemaVersion: "2.0",
			...
			Context:       manifest.Context{Project: projectDir},
			Claude:        manifest.Claude{},
			Tmux:          manifest.Tmux{SessionName: "test-tmux-1"},
		}
		...
		// Detect and associate with auto-apply
		...
		if !associated {
			t.Error("expected UUID to be associated")
		}

		// Verify manifest was updated
		if m.Claude.UUID != "uuid-auto-detect" {
			t.Errorf("expected UUID 'uuid-auto-detect', got '%s'", m.Claude.UUID)
		}
		...
		if reloaded.Claude.UUID != "uuid-auto-detect" {
			t.Errorf("expected persisted UUID 'uuid-auto-detect', got '%s'", reloaded.Claude.UUID)
		}
	})
