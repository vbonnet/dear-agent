...
func testSessionResume(m *manifest.Manifest, timeout time.Duration) (string, error) {
	...
	if timeout > 0 {
		if err := tmux.WaitForProcessReady(sessionName, "claude", timeout); err != nil {
			output, captureErr := capturePane(sessionName)
			if captureErr != nil {
				...
				Issues: []Issue{
					{
						...
						Fix:         "Run csm unlock to remove stale lock",
						AutoFixable: false,
					},
				},
				Manifest: m,
...
func classifyResumeError(output string, err error) *Issue {
	...
	case strings.Contains(output, "TypeError: Cannot read properties of undefined"):
		return &Issue{
			...
			Fix:         "Run 'csm doctor --validate --fix' to backfill from JSONL metadata",
			AutoFixable: true,
		}
	...
	case strings.Contains(output, "No messages returned"):
		return &Issue{
			...
			Fix:         "Run 'csm doctor --validate --fix' to update manifest version",
			AutoFixable: true,
		}
	...
	case strings.Contains(output, "summary entries") || strings.Contains(output, "summaries"):
		return &Issue{
			...
			Fix:         "Run 'csm doctor --validate --fix' to reorder JSONL (creates backup)",
			AutoFixable: true,
		}
	...
	case output == "" || strings.TrimSpace(output) == "":
		return &Issue{
			...
			Fix:         "Run 'csm doctor --validate --fix' to update working directory from JSONL",
			AutoFixable: true,
		}
