...
import (
	...
	"github.com/vbonnet/ai-tools/claude-session-manager/internal/eventbus"
)
...
func NewWatcher(hub eventbus.Broadcaster, incidentsFile string, escalationWindow time.Duration) *Watcher {
	if incidentsFile == "" {
		...
		if err != nil {
			...
			incidentsFile = filepath.Join(home, ".agm", "astrocyte", "incidents.jsonl")
		}
	}
...
func (w *Watcher) Start() error {
	...
	log.Printf("✓ Astrocyte watcher started (file: %s, window: %v)", w.incidentsFile, w.tracker.window)
...
func (w *Watcher) Stop() {
	close(w.shutdown)
	w.wg.Wait()
	log.Printf("✓ Astrocyte watcher stopped")
}
...
func (w *Watcher) initializeFilePosition() error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			w.lastPosition = 0
			log.Printf("✓ Incidents file does not exist yet, will watch for creation: %s", w.incidentsFile)
			return nil
		}
	...
	w.lastPosition = pos
	log.Printf("✓ Initialized file position: %d bytes", pos)
	...
	for {
		...
		case <-ticker.C:
			if err := w.checkForNewIncidents(); err != nil {
				log.Printf("⚠️  Error checking for new incidents: %v", err)
			}
		}
	}
}
...
func (w *Watcher) checkForNewIncidents() error {
	...
	if currentSize < w.lastPosition {
		log.Printf("⚠️  Incidents file truncated, resetting position")
		w.lastPosition = 0
	}
	...
	for scanner.Scan() {
		...
		if err := json.Unmarshal([]byte(line), &incident); err != nil {
			log.Printf("⚠️  Failed to parse incident: %v", err)
			continue
		}

		// Process the incident
		if err := w.processIncident(&incident); err != nil {
			log.Printf("⚠️  Failed to process incident: %v", err)
			continue
		}
	...
	if lineCount > 0 {
		log.Printf("✓ Processed %d new incident(s)", lineCount)
	}
...
func (w *Watcher) processIncident(incident *AstrocyteIncident) error {
	...
	if incident.SessionID == "" {
		log.Printf("⚠️  Skipping incident with empty session_id")
		return nil
	}

	// Check if we should publish based on time-windowing
	if !w.tracker.ShouldPublish(incident.SessionID) {
		log.Printf("⏭  Skipping duplicate escalation for session %s (within %v window)",
			incident.SessionID, w.tracker.window)
		return nil
	}
	...
	log.Printf("📡 Published escalation event: session=%s, type=%s, symptom=%s",
		incident.SessionID, payload.EscalationType, incident.Symptom)
