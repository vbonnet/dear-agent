...
func (c *EventBusClient) Connect(url string) error {
	...
	if err := c.connectWebSocket(); err != nil {
		log.Printf("WebSocket connection failed: %v, falling back to HTTP polling", err)
		c.httpFallback = true
		go c.httpPollLoop()
...
func (c *EventBusClient) attemptReconnect() {
	...
	for {
		...
		log.Printf("Successfully reconnected to event bus")
...
func (c *EventBusClient) pollHTTPEvents() {
	...
	if err != nil {
		log.Printf("HTTP polling failed: %v", err)
		return
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		log.Printf("HTTP polling returned status %d", resp.StatusCode)
		return
	}
