...
// TestGeminiTranslator_RunHook tests RunHook always returns ErrNotSupported
func TestGeminiTranslator_RunHook(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Should always return ErrNotSupported
			if !errors.Is(err, ErrNotSupported) {
				t.Errorf("expected ErrNotSupported, got %v", err)
			}

			// Should NOT call client (no API call made)
			if len(mock.CallLog) != 0 {
				t.Errorf("expected 0 calls (hook not supported), got %d", len(mock.CallLog))
			}
		})
	}
}
...
func TestErrorWrapping(t *testing.T) {
	t.Run("ErrAPIFailure wrapping", func(t *testing.T) {
		...
		// Should unwrap to get more details
		if err == nil {
			t.Fatal("err is nil")
		}
...
func BenchmarkGeminiTranslator_SetDirectory(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = translator.SetDirectory(ctx, "conv-123", "/home/user/project")
	}
}
...
func Example() {
	...
	if err := translator.SetDirectory(ctx, "conv-123", "/home/user/project"); err != nil {
		fmt.Println("Set directory failed:", err)
		return
	}

	// Run hook (returns ErrNotSupported for Gemini)
	if err := translator.RunHook(ctx, "conv-123", "/csm-assoc"); err != nil {
		if errors.Is(err, ErrNotSupported) {
