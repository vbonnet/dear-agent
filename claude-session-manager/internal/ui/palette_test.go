...
func TestDefaultPalette(t *testing.T) {
	...
	if p.Stale != lipgloss.Color("9") {
		t.Errorf("Expected Stale to be 9 (Bright Red), got %s", p.Stale)
	}
...
func TestLightPalette(t *testing.T) {
	...
	if p.Stale != lipgloss.Color("1") {
		t.Errorf("Expected Stale to be 1 (Red), got %s", p.Stale)
	}
...
func TestGetPalette(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "csm theme returns DefaultPalette",
			themeName: "csm",
			expected:  DefaultPalette(),
		},
		{
			name:      "csm-light theme returns LightPalette",
			themeName: "csm-light",
...
func TestSemanticConsistency(t *testing.T) {
	...
	// Warning should match Stopped (both indicate caution)
	if p.Warning != p.Stopped {
		t.Errorf("Warning (%s) should match Stopped (%s) for consistency", p.Warning, p.Stopped)
	}

	// Error should match Stale (both indicate problem)
	if p.Error != p.Stale {
		t.Errorf("Error (%s) should match Stale (%s) for consistency", p.Error, p.Stale)
	}
