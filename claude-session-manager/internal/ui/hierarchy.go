...
func FormatTableWithHierarchy(nodes []*db.SessionNode, tmux session.TmuxInterface) string {
	...
	switch layout {
	case LayoutMinimal:
		hierarchyOutput = renderHierarchyMinimal(nodes, statuses)
	case LayoutCompact:
		hierarchyOutput = renderHierarchyCompact(nodes, statuses)
	case LayoutFull:
		hierarchyOutput = renderHierarchyFull(nodes, statuses, showTmuxColumn, maxWidths)
	}
	...
	if len(groups["stale"]) > 0 {
		statusCounts = append(statusCounts, fmt.Sprintf("%d stale", len(groups["stale"])))
	}
...
func renderHierarchyMinimal(nodes []*db.SessionNode, statuses map[string]session.StatusInfo) string {
	...
	renderNode = func(node *db.SessionNode, prefix string, isLast bool) {
		...
		if statusInfo.Status == "active" {
			...
			// Check if stale based on UpdatedAt timestamp
			// TODO: Consider adding LastActivity to StatusInfo if needed
			status = "stopped"
...
func renderHierarchyCompact(nodes []*db.SessionNode, statuses map[string]session.StatusInfo) string {
	var result bytes.Buffer
...
func renderHierarchyFull(nodes []*db.SessionNode, statuses map[string]session.StatusInfo, showTmuxColumn bool, widths columnWidths) string {
	var result bytes.Buffer
