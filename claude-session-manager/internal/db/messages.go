...
type MessageOptions struct {
	...
	Agent  string    // Filter by agent ("claude", "gemini", "gpt", or empty for all)
...
func (db *DB) CreateMessage(sessionID string, msg *conversation.Message) error {
	...
	if msg.Agent == "" {
		return fmt.Errorf("message agent cannot be empty")
	}
	...
	query := `
		INSERT INTO messages (
			session_id, timestamp, role, agent, content,
...
func (db *DB) GetMessages(sessionID string, opts *MessageOptions) ([]*conversation.Message, error) {
	...
	query := `
		SELECT message_id, timestamp, role, agent, content,
			input_tokens, output_tokens
		FROM messages
		WHERE session_id = ?
	`
