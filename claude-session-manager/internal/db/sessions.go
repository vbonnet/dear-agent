...
type SessionFilter struct {
	...
	Agent           string    // Filter by agent type ("claude", "gemini", "gpt")
...
func (db *DB) CreateSession(session *manifest.Manifest) error {
	...
	query := `
		INSERT INTO sessions (
			...
			agent, context_project, context_purpose, context_tags, context_notes,
...
func (db *DB) UpdateSession(session *manifest.Manifest) error {
	...
	query := `
		...
		SET name = ?, schema_version = ?, updated_at = ?, lifecycle = ?,
			agent = ?, context_project = ?, context_purpose = ?, context_tags = ?,
