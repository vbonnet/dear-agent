...
import (
	...
	"github.com/vbonnet/ai-tools/claude-session-manager/internal/conversation"
...
func TestDatabaseTransaction(t *testing.T) {
	...
	_, err = tx.Exec(`
		INSERT INTO sessions (session_id, name, schema_version, created_at, updated_at, lifecycle, agent)
		VALUES (?, ?, ?, ?, ?, ?, ?)
	`, "test-session", "Test", "2.0", time.Now(), time.Now(), "", "claude")
	require.NoError(t, err)
...
func TestSessionCRUD(t *testing.T) {
	...
	t.Run("get session", func(t *testing.T) {
		...
		assert.Equal(t, session.Agent, retrieved.Agent)
	...
	t.Run("list sessions", func(t *testing.T) {
		...
		// Filter by agent
		filter = &SessionFilter{Agent: "gemini"}
		sessions, err = db.ListSessions(filter)
		require.NoError(t, err)
		assert.Len(t, sessions, 1)
		assert.Equal(t, "gemini", sessions[0].Agent)
...
func createTestMessage(role, agent string) *conversation.Message {
...
func TestMessageCRUD(t *testing.T) {
	...
	t.Run("create message", func(t *testing.T) {
		msg := createTestMessage("user", "claude")
	...
	t.Run("get messages", func(t *testing.T) {
		...
		msg2 := createTestMessage("assistant", "claude")
		err := db.CreateMessage("msg-test-session", msg2)
		require.NoError(t, err)

		msg3 := createTestMessage("user", "gemini")
	...
	t.Run("filter messages by agent", func(t *testing.T) {
		opts := &MessageOptions{Agent: "gemini"}
		messages, err := db.GetMessages("msg-test-session", opts)
		require.NoError(t, err)
		assert.Len(t, messages, 1)
		assert.Equal(t, "gemini", messages[0].Agent)
	})
...
func createTestEscalation(eventType string) *activities.EscalationEvent {
	return &activities.EscalationEvent{
		Type:        eventType,
		Pattern:     `(?i)error:`,
