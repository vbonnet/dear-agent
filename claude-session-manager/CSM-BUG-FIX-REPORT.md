...
**The Problem:**
- When `csm sync` ran, it found multiple active tmux sessions without Claude UUIDs
...
- Prompts user to manually associate each session using `csm associate <session-name>`
...
The `csm doctor` command now detects:
...
2. **Sessions sharing the same Claude UUID**
   - Critical issue that breaks `csm resume`
   - Shows which sessions share which UUIDs
   - Recommends using `csm associate` to fix
...
⚠ Found 1 Claude UUID(s) shared by multiple sessions
  ...
  Recommendation: Each session should have a unique Claude UUID
    Use 'csm associate <session-name>' to assign correct UUIDs
```
...
### Fixed
✅ `csm sync` no longer assigns the same UUID to multiple sessions
✅ `csm doctor` detects duplicate UUIDs and directories
✅ Old format session directories archived
✅ No more duplicate session entries in `csm list`

### Remaining Issues (Require Manual Intervention)
⚠️ 12 sessions still share UUID `70875f41-e829-47e3-81a7-9670422f9c36`
⚠️ 1 session (csm-close) has empty UUID
...
- If it's NOT the correct session: Run `csm associate <session-name>` to assign the correct UUID
- If the session is no longer needed: Archive it

**For session with empty UUID (csm-close):**
Run: `csm associate csm-close` to link it to the current Claude conversation
...
3. Run `csm doctor` regularly to detect future issues
...
4. Add validation in `csm new` to prevent UUID conflicts
...
But this hook doesn't exist! This is why sessions created via `csm new` have empty UUIDs.

**Solution Options:**
1. Implement the SessionStart hook (preferred)
2. Update `csm new` to capture UUID immediately after starting Claude
3. Remove the misleading message and document that `csm associate` is required

## Conclusion

The critical UUID bug has been fixed in the code, preventing future occurrences. However, existing sessions require manual remediation. The enhanced `csm doctor` command provides clear diagnostics and recommendations for fixing these issues.
