...
func runSearch(cmd *cobra.Command, args []string) error {
	...
	if cachedResults := searchCache.Get(query); cachedResults != nil {
		...
		return handleSearchResults(query, cachedResults)
	}
	...
	return handleSearchResults(query, results)
}

func handleSearchResults(query string, results []llm.SearchResult) error {
	switch len(results) {
	case 0:
	...
	case 1:
		...
		return restoreArchivedSession(archivedSession)
