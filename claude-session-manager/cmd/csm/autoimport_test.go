...
func TestGenerateTmuxName(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			project:          "/home/user/my@project#2024",
			existingSessions: []string{},
...
func TestOfferToImportOrphanedSession_NoHistory(t *testing.T) {
	...
	_, _, err := offerToImportOrphanedSession("test-uuid")
	if err == nil {
...
`
	...
	_, _, err := offerToImportOrphanedSession("nonexistent-uuid")
	if err == nil {
...
{"sessionId":"test-uuid-2222","project":"/home/user/another-test","timestamp":1733500001000}
`
	if err := os.WriteFile(historyPath, []byte(historyContent), 0600); err != nil {
		t.Fatalf("Failed to create history: %v", err)
	}
	...
	_, _, err := offerToImportOrphanedSession("test")
	if err == nil {
...
func TestResolveSessionIdentifier_WithAutoImport(t *testing.T) {
	...
	historyContent := fmt.Sprintf(`{"sessionId":"%s","project":"/home/user/orphaned-project","timestamp":%d}
`, testUUID, time.Now().UnixMilli())
	...
	_, _, err := resolveSessionIdentifier("orphan-1234")
