...
func TestHeaderFormat(t *testing.T) {
	...
	// Verify format: "csm-reaper <version> (<path>)"
	if !strings.HasPrefix(header, "csm-reaper ") {
		t.Errorf("Header should start with 'csm-reaper ', got: %s", header)
	}
...
func formatHeader(version, executable string) string {
	return "csm-reaper " + version + " (" + executable + ")"
}
