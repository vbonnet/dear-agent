...
func TestValidateParentSession(t *testing.T) {
	...
	parentManifestPath := filepath.Join(parentManifestDir, "manifest.yaml")
...
func TestFindManifestByTmuxName(t *testing.T) {
	...
	require.NoError(t, manifest.Write(manifestPath, testManifest))
...
func TestCreateChildCommand_InheritanceLogic(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			validateChild: func(t *testing.T, child *manifest.Manifest) {
				// Always inherited
				assert.Equal(t, parentManifest.Agent, child.Agent, "should inherit agent")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			childManifest := &manifest.Manifest{
				...
				Agent: parentManifest.Agent, // Always inherited
			}
...
func TestCreateChildCommand_ErrorHandling(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			scenario:      "Backend (tmux/temporal) is not available",
...
func TestCreateChildCommand_Coverage(t *testing.T) {
	...
	t.Log("  • Full command execution (requires tmux/temporal)")
