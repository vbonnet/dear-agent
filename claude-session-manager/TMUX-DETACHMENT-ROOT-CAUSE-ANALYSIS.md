...
2. **Delete old csm binaries** (requires permissions):
   ```bash
   rm /home/user/go/bin/csm
   rm /home/user/go/bin/csm-attach-wrapper
   rm /home/user/go/bin/csm-agent-wrapper
   rm /home/user/go/bin/csm-reaper
   ```
...
### Manual Verification
1. Create test session: `agm session new test-recovery --agent claude --detached`
...
3. Check for any remaining csm references in active code
