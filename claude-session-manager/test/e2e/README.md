...
#### Execute Commands
```
exec csm list                  # Run command, expect success
! exec csm invalid-command     # Run command, expect failure
```
...
exec csm new --detached test-session-1
...
# Clean up
exec csm archive test-session-1 --force
```
