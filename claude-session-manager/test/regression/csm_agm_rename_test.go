...
// TestRegressionDocumentation ensures CSM→AGM rename regressions are documented
...
// TestNoCSMReferencesInUserDocs ensures user-facing docs use AGM not CSM
func TestNoCSMReferencesInUserDocs(t *testing.T) {
	...
	// Except in historical context or "renamed from CSM" mentions
	...
	for _, docPath := range userDocs {
		t.Run(filepath.Base(docPath), func(t *testing.T) {
			...
			for _, pattern := range forbiddenPatterns {
				// Allow mentions in "renamed from CSM" or historical notes
				...
				if strings.Contains(contentStr, strings.ToLower(pattern)) {
					t.Errorf("Found CSM reference '%s' in %s - should use AGM branding instead", pattern, docPath)
				}
			}
		})
	}
		...
		t.Run(filepath.Base(docPath), func(t *testing.T) {
			...
			if !strings.Contains(contentStr, "migration") &&
				...
				assert.NotContains(t, contentStr, "~/.config/csm",
					"%s should not reference old CSM config path", docPath)
			}
		})
	}
}
	...
	for _, docPath := range docs {
		t.Run(filepath.Base(docPath), func(t *testing.T) {
			...
			if strings.Contains(contentStr, "theme:") {
				...
				// Should NOT use old CSM theme names (except in migration notes)
				if !strings.Contains(contentStr, "migration") {
					assert.False(t,
						strings.Contains(contentStr, `theme: "csm"`) ||
							strings.Contains(contentStr, `theme: 'csm'`),
						"%s should not reference old CSM theme names", docPath)
				}
			}
		})
	}
