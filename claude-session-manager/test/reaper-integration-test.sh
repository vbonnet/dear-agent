#!/usr/bin/env bash
# Integration test for csm-reaper with agm send fix
...
REAPER_LOG="/tmp/csm-reaper-${TEST_SESSION}.log"
...
if ! command -v csm-reaper >/dev/null 2>&1; then
    error "csm-reaper binary not found in PATH"
    exit 1
fi

log "Starting integration test for csm-reaper with agm send"
