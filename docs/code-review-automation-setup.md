...
5. **Roll out to other repos** — see `infra/claude_review.tf` for the policy
   ...
   credentials, a fresh process-local `TF_ENCRYPTION` method (the checked-in
   configuration rejects bare plan and apply invocations — see
   [Plan encryption](../infra/README.md#plan-encryption)), a GitHub token with
