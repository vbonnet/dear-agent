...
// files (simple.go, tiktoken.go). Returns an error if a tokenizer with the
// same name is already registered.
//
// Thread-safe: Can be called concurrently (though typically only called from init()).
func Register(t Tokenizer) error {
	mu.Lock()
	defer mu.Unlock()

	name := t.Name()
	if _, exists := registry[name]; exists {
		return fmt.Errorf("tokenizer already registered: %s", name)
	}
	registry[name] = t
	return nil
}
