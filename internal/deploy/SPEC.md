...
**DEP-12** When an operator installs a root-owned hook helper, the system shall require exact artifact and fixed-bootstrap digest confirmation, reject a passwordless probe, and stage, verify, and atomically activate the helper beneath verified root-owned non-writable non-symlink ancestry in one non-caching privileged command; for the SPEC contract helper that transaction shall first publish or validate an exact root-owned digest-suffixed hard link without replacing or cleaning up any existing content-addressed identity, shall validate and reuse an exact winner when a concurrent same-digest publication wins the no-clobber link race, shall accept only that exact identity and remove its remaining staging alias when another same-digest transaction wins the stable activation race, and shall reject a mismatched existing digest leaf before changing the stable cooperative path.
