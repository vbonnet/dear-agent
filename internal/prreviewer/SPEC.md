...
**PRV-03** When optional configuration is omitted, the system shall apply the documented defaults for inspection limit, review event, provider commands, secondary attempts, state path, and clock.
...
**PRV-07** When the secondary provider fails, the system shall retry up to the configured attempt count and then continue with a primary-only review body.

**PRV-08** When the secondary provider is unavailable, the system shall record its unavailability and last error in the posted review body.
...
**PRV-10** When a review is posted successfully, the system shall record the reviewed head SHA so a later pass skips the unchanged pull request.

**PRV-11** When the state file does not exist, the system shall treat the recorded state as empty rather than failing the pass.

**PRV-12** When state is persisted, the system shall create the state directory with owner-only permissions and write the state file with owner-only permissions.

**PRV-13** When a review body is composed, the system shall include the repository, pull request number, review timestamp, and one section per provider.
