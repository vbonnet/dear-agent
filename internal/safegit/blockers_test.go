...
func TestClassifyBlockers_OutdatedUnresolvedThreadBlocks(t *testing.T) {
	...
	if !strings.Contains(bs[0].Fix, "resolve-review-threads reply-resolve <threadId> --body-file reply.md") {
		t.Errorf("thread fix must lead with the body-file-only reply-resolve form, got: %s", bs[0].Fix)
	}
	if strings.Contains(bs[0].Fix, `reply-resolve <threadId> "`) {
		t.Errorf("thread fix must not put a reply body in shell argv, got: %s", bs[0].Fix)
	}
