...
**SAFEGIT-13** When post-merge cleanup attempts linked-worktree removal and local branch deletion, the system shall execute both Git operations from the stable primary worktree with caller cancellation and a bounded per-command deadline, attempt worktree removal before conservative branch deletion, and report cleanup failures as warnings without changing the completed provider merge result.
